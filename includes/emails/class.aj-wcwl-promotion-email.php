...
if ( ! class_exists( 'AJ_WCWL_Promotion_Email' ) ) {
	/**
	 * WooCommerce Wishlist Promotion Email
	 *
	 * @since 2.0.7
	 */
	class AJ_WCWL_Promotion_Email extends WC_Email {
		 ...
		 *
		 * @return \AJ_WCWL_Promotion_Email
		 * @since 2.0.7
		 */
		public function __construct() {
			$this->id          = 'aj_wcwl_promotion_mail';
			$this->title       = __( 'Wishlist promotional email', 'aj-woocommerce-wishlist' );
			$this->description = __( 'This email is sent to users to inform them about a promotion on a product in their wishlist', 'aj-woocommerce-wishlist' );

			$this->heading = __( 'There is a deal for you!', 'aj-woocommerce-wishlist' );
			$this->subject = __( 'A product of your wishlist is on sale', 'aj-woocommerce-wishlist' );
		...
		public function trigger( $receivers_ids, $additional_info ) {
			...
			if ( ! empty( $additional_info['product_id'] ) ) {
				...
				add_filter( 'aj_wcwl_promotional_email_send_result', '__return_false' );
			...
			update_post_meta( dido_get_product_id( $this->object ), 'last_promotional_email', time() );

			add_filter( 'aj_wcwl_promotional_email_send_result', $result ? '__return_true' : '__return_false' );
		}
		...
		public function get_custom_content_html() {
			...
			$image_size = apply_filters( 'aj_wcwl_promotional_email_thumbnail_item_size', array( 32, 32 ) );
			...
			$replace = array_merge(
				array(
					...
					'product_image'   => $this->object ? apply_filters( 'aj_wcwl_promotional_email_item_thumbnail', '<div style="margin-bottom: 5px"><img src="' . ( $this->object->get_image_id() ? current( wp_get_attachment_image_src( $this->object->get_image_id(), 'thumbnail' ) ) : wc_placeholder_img_src() ) . '" alt="' . __( 'Product image', 'aj-woocommerce-wishlist' ) . '" height="' . esc_attr( $image_size[1] ) . '" width="' . esc_attr( $image_size[0] ) . '" style="vertical-align:middle; margin-' . ( is_rtl() ? 'left' : 'right' ) . ': 10px;" /></div>', $this->object ) : '',
					...
					'add_to_cart_url' => $this->object ? esc_url( add_query_arg( 'add-to-cart', dido_get_product_id( $this->object ), $this->object->get_permalink() ) ) : '',
					'wishlist_url'    => AJ_WCWL()->get_wishlist_url(),
				),
				! isset( $this->coupon ) ? array() : array(
					'coupon_code'   => dido_get_prop( $this->coupon, 'code' ),
		...
		public function init_form_fields() {
			$this->form_fields = array(
				...
				'content_html' => array(
					...
					'description' => sprintf( __( 'This field lets you modify the main content of the HTML email. You can use the following placeholders: %s', 'aj-woocommerce-wishlist' ), self::get_placeholder_text( 'html' ) ),
					'placeholder' => '',
				...
				'content_text' => array(
					...
					'type'        => 'textarea',
					// translators: 1. Available placeholders.
					'description' => sprintf( __( 'This field lets you modify the main content of the text email. You can use the following placeholders: %s', 'aj-woocommerce-wishlist' ), self::get_placeholder_text( 'plain' ) ),
					'placeholder' => '',
					'css'         => 'min-height: 250px;',
...
return new AJ_WCWL_Promotion_Email();
