...
if ( ! class_exists( 'AJ_WCWL_Back_In_Stock_Email' ) ) {
	/**
	 * Back in stock email
	 *
	 * @since 1.0.0
	 */
	class AJ_WCWL_Back_In_Stock_Email extends WC_Email {
		 ...
		 *
		 * @return \AJ_WCWL_Back_In_Stock_Email
		 * @since 3.0.0
		 */
		public function __construct() {
			$this->id          = 'aj_wcwl_back_in_stock';
			$this->title       = __( 'Wishlist "Back in stock" email', 'aj-woocommerce-wishlist' );
			$this->description = __( 'This email is sent to customers when an item of their wishlist is back in stock', 'aj-woocommerce-wishlist' );

			$this->heading = __( 'An item of your wishlist is back in stock!', 'aj-woocommerce-wishlist' );
			$this->subject = __( 'An item of your wishlist is back in stock!', 'aj-woocommerce-wishlist' );
		 ...
		 * @param \AJ_WCWL_Wishlist_Item[] $items List of wishlist items.
		 *
		 * @return void
		 * @since 3.0.0
		 */
		...
		public function get_custom_content_html() {
			...
			$replace = array(
				...
				'unsubscribe_link' => sprintf( '<a href="%s">%s</a>', AJ_WCWL()->get_unsubscribe_link( $this->user->ID ), apply_filters( 'aj_wcwl_unsubscribe_link_label', __( 'unsubscribe', 'aj-woocommerce-wishlist' ), $this ) ),
			);
			...
			$custom_content = apply_filters( 'aj_wcwl_custom_html_content_' . $this->id, $this->format_string( stripcslashes( $this->content_html ) ), $this->object );
		...
		public function get_custom_content_plain() {
			...
			$replace = array(
				...
				'unsubscribe_url' => AJ_WCWL()->get_unsubscribe_link( $this->user->ID ),
			);
			...
			$custom_content = apply_filters( 'aj_wcwl_custom_text_content_' . $this->id, $this->format_string( stripcslashes( $this->content_text ) ), $this->object );
		...
		public function get_products_table( $plain_text = false ) {
			if ( $plain_text ) {
				return aj_wcwl_get_template( 'emails/plain/products-list.php', array( 'items' => $this->items ), true );
			} else {
				return aj_wcwl_get_template( 'emails/products-table.php', array( 'items' => $this->items ), true );
			}
		}
		...
		public function init_form_fields() {
			...
			$this->form_fields = array(
				'enabled' => array(
					'title'   => __( 'Enable/Disable', 'aj-woocommerce-wishlist' ),
					'type'    => 'checkbox',
					'label'   => __( 'Enable this email notification', 'aj-woocommerce-wishlist' ),
					'default' => 'no',
				),
				...
				'product_exclusions' => array(
					'title'       => __( 'Product exclusions', 'aj-wooommerce-wishlist' ),
					'type'        => 'multiselect',
					'description' => __( 'Select the products for which you don\'t want to send a reminder email', 'aj-woocommerce-wishlist' ),
					'class'       => 'wc-product-search',
					'options'     => $exclusions_options,
				),

				'category_exclusions' => array(
					'title'       => __( 'Category exclusions', 'aj-wooommerce-wishlist' ),
					'type'        => 'multiselect',
					'class'       => 'wc-enhanced-select',
					'description' => __( 'Select the product categories for which you don\'t want to send a reminder email', 'aj-woocommerce-wishlist' ),
				...
				'content_html' => array(
					'title'       => __( 'Email HTML Content', 'aj-woocommerce-wishlist' ),
					'type'        => 'textarea',
					'description' => __( 'This field lets you modify the main content of the HTML email. You can use the following placeholders: <code>{user_name}</code> <code>{user_email}</code> <code>{user_first_name}</code> <code>{user_last_name}</code> <code>{products_table}</code> <code>{unsubscribe_link}</code>', 'aj-woocommerce-wishlist' ),
					'placeholder' => '',
				...
				'content_text' => array(
					'title'       => __( 'Email text content', 'aj-woocommerce-wishlist' ),
					'type'        => 'textarea',
					'description' => __( 'This field lets you modify the main content of the text email. You can use the following placeholders: <code>{user_name}</code> <code>{user_email}</code> <code>{user_first_name}</code> <code>{user_last_name}</code> <code>{products_list}</code> <code>{unsubscribe_url}</code>', 'aj-woocommerce-wishlist' ),
					'placeholder' => '',
					'css'         => 'min-height: 250px;',
...
return new AJ_WCWL_Back_In_Stock_Email();
