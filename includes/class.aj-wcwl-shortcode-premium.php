...
if ( ! class_exists( 'AJ_WCWL_Shortcode_Premium' ) ) {
	/**
	 * AJ WCWL Shortcodes Premium
	 *
	 * @since 1.0.0
	 */
	class AJ_WCWL_Shortcode_Premium {

		/**
		 * Constructor
		 *
		 * @return \AJ_WCWL_Shortcode_Premium
		 * @since 2.0.0
		 */
		public function __construct() {
			...
			add_filter( 'aj_wcwl_available_wishlist_views', array( 'AJ_WCWL_Shortcode_Premium', 'add_wishlist_views' ) );
			add_filter( 'aj_wcwl_wishlist_params', array( 'AJ_WCWL_Shortcode_Premium', 'wishlist_view' ), 5, 6 );
			add_filter( 'aj_wcwl_wishlist_params', array( 'AJ_WCWL_Shortcode_Premium', 'wishlist_create' ), 10, 6 );
			add_filter( 'aj_wcwl_wishlist_params', array( 'AJ_WCWL_Shortcode_Premium', 'wishlist_manage' ), 15, 6 );
			add_filter( 'aj_wcwl_wishlist_params', array( 'AJ_WCWL_Shortcode_Premium', 'wishlist_search' ), 20, 6 );

			// Filters applied to add params to add-to-wishlist view.
			add_filter( 'aj_wcwl_add_to_wishlist_params', array( 'AJ_WCWL_Shortcode_Premium', 'add_to_wishlist_popup' ), 10, 2 );
		}
		...
		public static function init() {
			// register shortcodes.
			add_shortcode( 'aj_wcwl_show_public_wishlist', array( 'AJ_WCWL_Shortcode_Premium', 'show_public_wishlist' ) );

			// register gutenberg blocks.
			add_action( 'init', array( 'AJ_WCWL_Shortcode_Premium', 'register_gutenberg_blocks' ) );
		}
		...
		public static function register_gutenberg_blocks() {
			$blocks = array(
				'aj-wcwl-show-public-wishlist' => array(
					'style'          => 'aj-wcwl-main',
					'script'         => 'jquery-aj-wcwl',
					'title'          => _x( 'AJ public wishlist link', '[gutenberg]: block name', 'aj-woocommerce-wishlist' ),
					'description'    => _x( 'Shows all public wishlists', '[gutenberg]: block description', 'aj-woocommerce-wishlist' ),
					'shortcode_name' => 'aj_wcwl_show_public_wishlist',
		...
		public static function wishlist_view( $additional_params, $action, $action_params, $pagination, $per_page, $atts ) {
			...
			if ( ! empty( $additional_params['template_part'] ) && 'view' == $additional_params['template_part'] ) {
				...
				$layout = ! empty( $additional_params['layout'] ) ? $additional_params['layout'] : get_option( 'aj_wcwl_wishlist_layout', 'traditional' );
				...
				$show_ask_estimate_button = $wishlist && 'yes' == get_option( 'aj_wcwl_show_estimate_button' ) && $wishlist->current_user_can( 'ask_an_estimate' );
				$multi_wishlist = AJ_WCWL()->is_multi_wishlist_enabled();
				$additional_info = get_option( 'aj_wcwl_show_additional_info_textarea' ) == 'yes';
				$additional_info_label = get_option( 'aj_wcwl_additional_info_textarea_label' );
				$move_to_another_wishlist = get_option( 'aj_wcwl_show_move_to_another_wishlist' ) == 'yes';
				$move_to_another_wishlist_type = get_option( 'aj_wcwl_move_to_another_wishlist_type', 'select' );
				$move_to_another_wishlist_url = false;
				$show_cb = get_option( 'aj_wcwl_cb_show' ) == 'yes';
				$show_quantity = get_option( 'aj_wcwl_quantity_show' ) == 'yes';
				$show_price_variations = get_option( 'aj_wcwl_price_changes_show' ) == 'yes';
				$enable_add_all_to_cart = get_option( 'aj_wcwl_enable_add_all_to_cart' ) == 'yes';
				$enable_drag_n_drop = get_option( 'aj_wcwl_enable_drag_and_drop' ) == 'yes';
				...
				if ( $show_ask_estimate_button && $wishlist ) {
					...
					$ask_an_estimate_text = get_option( 'aj_wcwl_ask_an_estimate_label', __( 'Ask for an estimate', 'aj-woocommerce-wishlist' ) );

					if ( $additional_info ) {
						$ask_an_estimate_fields = aj_wcwl_maybe_format_field_array( get_option( 'aj_wcwl_ask_an_estimate_fields', array() ) );
					}

					$ask_an_estimate_style = get_option( 'aj_wcwl_ask_an_estimate_style' );
					$ask_an_estimate_classes = 'link' != $ask_an_estimate_style ? 'button btn' : '';

					if ( 'button_custom' == $ask_an_estimate_style ) {
						$ask_an_estimate_icon = get_option( 'aj_wcwl_ask_an_estimate_icon' );
						$ask_an_estimate_custom_icon = get_option( 'aj_wcwl_ask_an_estimate_custom_icon' );
						...
						if ( $ask_an_estimate_icon && ! in_array( $ask_an_estimate_icon, array( 'none', 'custom' ) ) ) {
							...
							$ask_an_estimate_alt_text = __( 'Ask for an estimate', 'aj-woocommerce-wishlist' );
				...
				$additional_params = array_merge(
					...
					array(
						...
						'users_wishlists' => AJ_WCWL()->get_wishlists(),
						...
						'show_update' => apply_filters( 'aj_wcwl_show_wishlist_update_button', $show_update, $wishlist ),
					)
				);
			}
		...
		public static function wishlist_create( $additional_params, $action, $action_params, $pagination, $per_page, $atts ) {
			/* === CREATE TEMPLATE === */
			if ( ! empty( $action ) && 'create' == $action && AJ_WCWL()->is_multi_wishlist_enabled() ) {
				/*
				...
				$page_title = get_option( 'aj_wcwl_wishlist_create_title' );
		...
		public static function wishlist_manage( $additional_params, $action, $action_params, $pagination, $per_page, $atts ) {
			/* === MANAGE TEMPLATE === */
			if ( ! empty( $action ) && 'manage' == $action && AJ_WCWL()->is_multi_wishlist_enabled() ) {
				/*
				...
				$layout = ! empty( $atts['layout'] ) ? $atts['layout'] : get_option( 'aj_wcwl_wishlist_manage_layout', 'traditional' );

				$page_title = get_option( 'aj_wcwl_wishlist_manage_title' );
				$show_number_of_items = get_option( 'aj_wcwl_manage_num_of_items_show' );
				$show_date_of_creation = get_option( 'aj_wcwl_manage_creation_date_show' );
				$show_download_as_pdf = get_option( 'aj_wcwl_manage_download_pdf_show' );
				$show_rename_wishlist = get_option( 'aj_wcwl_manage_rename_wishlist_show' );
				$show_delete_wishlist = get_option( 'aj_wcwl_manage_delete_wishlist_show' );

				// retrieve user wishlist.
				$user_wishlists = AJ_WCWL()->get_current_user_wishlists();
				...
				$additional_params['fragment_options'] = AJ_WCWL_Frontend()->format_fragment_options( $additional_params, 'wishlist_manage' );
			}
		...
		public static function wishlist_search( $additional_params, $action, $action_params, $pagination, $per_page, $atts ) {
			...
			if ( ! empty( $action ) && 'search' == $action ) {
				...
				$page_title = get_option( 'aj_wcwl_wishlist_search_title' );
				...
				if ( ! empty( $wishlist_search ) ) {
					...
					$count = AJ_WCWL_Wishlist_Factory::get_wishlists_count( $search_args );
					...
					if ( 'yes' == $pagination && $count > 1 ) {
						...
						if ( $pages > 1 ) {
							$page_links = paginate_links(
								array(
									'base' => esc_url( add_query_arg( array( 'paged' => '%#%' ), AJ_WCWL()->get_wishlist_url( 'search/' . $wishlist_search ) ) ),
									'format' => '?paged=%#%',
					...
					$search_results = AJ_WCWL_Wishlist_Factory::get_wishlists( $search_args );
				}
		...
		public static function add_to_wishlist_popup( $additional_info, $atts ) {
			$multi_wishlist = AJ_WCWL()->is_multi_wishlist_enabled();
			$show_popup = 'default' != get_option( 'aj_wcwl_modal_enable', 'yes' );
			$lists = AJ_WCWL()->get_current_user_wishlists();

			$is_single = aj_wcwl_is_single();
			$popup_title = __( 'Select a wishlist', 'aj-woocommerce-wishlist' );
			$label = apply_filters( 'aj_wcwl_button_popup_label', get_option( 'aj_wcwl_add_to_wishlist_popup_text' ) );
			...
			$disable_wishlist = get_option( 'aj_wcwl_disable_wishlist_for_unauthenticated_users' ) == 'yes';
			$show_exists_in_a_wishlist = get_option( 'aj_wcwl_after_add_to_wishlist_behaviour' ) != 'add';
			$show_count = get_option( 'aj_wcwl_show_counter' ) == 'yes' && $is_single;
			$show_view = $is_single;
			$add_to_wishlist_modal = get_option( 'aj_wcwl_modal_enable', 'yes' );
			...
			if ( ! $additional_info['exists'] && $multi_wishlist ) {
				$found_in_list = AJ_WCWL()->get_wishlist_for_product( $additional_info['product_id'] );
			...
			if ( 'remove' == $template_part ) {
				...
				$additional_info['label'] = apply_filters( 'aj_wcwl_remove_from_wishlist_label', __( '', 'aj-woocommerce-wishlist' ) );
			}
			...
			if ( 'move' == $template_part ) {
				...
				$additional_info['label'] = apply_filters( 'aj_wcwl_move_from_wishlist_label', __( 'Move &rsaquo;', 'aj-woocommerce-wishlist' ) );

				$popup_title = __( 'Move to another wishlist', 'aj-woocommerce-wishlist' );
			}
			...
			$additional_info = array_merge(
				...
				array(
					'template_part' => $template_part,
					'product_image' => '',
					'popup_title' => apply_filters( 'aj_wcwl_add_to_wishlist_popup_title', $popup_title, $template_part ),
					'lists' => $lists,
					'label_popup' => $label,
					'available_multi_wishlist' => $multi_wishlist,
					'show_exists' => $show_exists_in_a_wishlist,
					'link_popup_classes' => apply_filters( 'aj_wcwl_add_to_wishlist_popup_classes', $popup_classes ),
		...
		public static function show_public_wishlist() {
			...
			$wishlists = AJ_WCWL()->get_wishlists(
				array(
					'user_id' => false,
			...
			$template = aj_wcwl_get_template( 'wishlist-public-list.php', $atts, true );

			return apply_filters( 'aj_wcwl_public_wishlist_html', $template );

		}
	}
}

return new AJ_WCWL_Shortcode_Premium();
