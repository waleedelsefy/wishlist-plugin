...
if ( ! class_exists( 'AJ_WCWL_Popular_Table' ) ) {
	/**
	 * Admin view class. Create and populate "user with wishlists" table
	 *
	 * @since 1.0.0
	 */
	class AJ_WCWL_Popular_Table extends WP_List_Table {

		/**
		 * Class constructor method
		 *
		 * @return \AJ_WCWL_Popular_Table
		 * @since 2.0.0
		 */
		...
		public function column_name( $item ){
			...
			$actions = array(
				'ID' => $item['id'],
				'edit' => sprintf( '<a href="%s" title="%s">%s</a>', $product_edit_url, __( 'Edit this item', 'aj-woocommerce-wishlist' ), __( 'Edit', 'aj-woocommerce-wishlist' ) ),
				'view_users' => sprintf( '<a href="%s" title="%s">%s</a>', esc_url( add_query_arg( array( 'page' => 'aj_wcwl_panel', 'tab' => 'popular', 'action' => 'show_users', 'product_id' => $item['id'] ), admin_url( 'admin.php' ) ) ), __( 'View a list with users that have added this product to their wishlist', 'aj-woocommerce-wishlist' ), __( 'View users', 'aj-woocommerce-wishlist' ) ),
				'view_product' => sprintf( '<a href="%s" title="%s" rel="permalink">%s</a>', $product_url, __( 'View Product', 'aj-woocommerce-wishlist' ), __( 'View Product', 'aj-woocommerce-wishlist' ) ),
			);
			$row_actions = $this->row_actions( $actions );

			$column_content = sprintf( '%s<div class="product-details"><strong><a class="row-title" href="%s">%s</a></strong>%s</div>', $product->get_image( 'thumbnail' ), esc_url( add_query_arg( array( 'page' => 'aj_wcwl_panel', 'tab' => 'popular', 'action' => 'show_users', 'product_id' => $item['id'] ), admin_url( 'admin.php' ) ) ), $product_name, $row_actions );
		...
		public function column_count( $item ){
			...
			return sprintf( '<a href="%s">%d</a>', esc_url( add_query_arg( array( 'page' => 'aj_wcwl_panel', 'tab' => 'popular', 'action' => 'show_users', 'product_id' => $item['id'] ), admin_url( 'admin.php' ) ) ), $column_content );
		}
		...
		public function column_last_sent( $item ) {
			...
			if( ! $last_sent ){
				$column = __( 'N/A', 'aj-woocommerce-wishlist' );
			}
			else{
		...
		public function column_actions( $item ){
			...
			$product_edit_url = get_edit_post_link( $item['id'] );
			$view_users_url = esc_url( add_query_arg( array( 'page' => 'aj_wcwl_panel', 'tab' => 'popular', 'action' => 'show_users', 'product_id' => $item['id'] ), admin_url( 'admin.php' ) ) );
			$export_users_url = esc_url( add_query_arg( array( 'action' => 'export_users', 'product_id' => $item['id'] ), admin_url( 'admin.php' ) ) );
			$send_promotional_email = esc_url( add_query_arg( array( 'page' => 'aj_wcwl_panel', 'tab' => 'popular', 'action' => 'send_promotional_email', 'product_id' => $item['id'] ), admin_url( 'admin.php' ) ) );
			$product_name = $item['post_title'];

			$actions = array(
				sprintf( '<a class="button-primary send create-promotion" href="%s" title="%2$s" data-product_id="%3$s"><i class="material-icons">mail_outline</i>%2$s</a>', $send_promotional_email, __( 'Create promotion', 'aj-woocommerce-wishlist' ), $item['id'] ),
				sprintf( '<a class="button-secondary user" href="%s" title="%2$s"><i class="material-icons">people</i></a>', $view_users_url, __( 'View users that have added this product to their wishlist', 'aj-woocommerce-wishlist' ) ),
				sprintf( '<a class="button-secondary export" href="%s" title="%2$s"><i class="material-icons">save_alt</i></a>', $export_users_url, __( 'Export users that have added this product to their wishlist', 'aj-woocommerce-wishlist' ) ),
			);
		...
		public function get_columns(){
			$columns = array(
				...
				'category'  => __( 'Category', 'aj-woocommerce-wishlist' ),
				'count'     => __( 'Wishlist count', 'aj-woocommerce-wishlist' ),
				'last_sent' => __( 'Last promotional email sent', 'aj-woocommerce-wishlist' ),
				'actions'   => __( 'Actions', 'aj-woocommerce-wishlist' )
			);
			return $columns;
		}
		...
		public function display() {
			...
			do_action(  'aj_wcwl_after_popular_table' );
		}
		...
		protected function extra_tablenav( $which ) {
			...
			dido_add_select2_fields( array(
				'class'            => 'wc-product-search',
				'name'             => '_product_id',
				'data-placeholder' => __( 'Select a product', 'aj-woocommerce-wishlist' ),
			...
			?>
			<input style="min-width: 200px; vertical-align:middle;" class="date-picker" type="text" name="_from" value="<?php echo esc_attr( $from ) ?>" placeholder="<?php _e( 'From:', 'aj-woocommerce-wishlist' ) ?>"/>
			<input style="min-width: 200px; vertical-align:middle;" class="date-picker" type="text" name="_to" value="<?php echo esc_attr( $to ) ?>" placeholder="<?php _e( 'To:', 'aj-woocommerce-wishlist' ) ?>"/>
			<input type="hidden" name="status" class="post_status_page" value="<?php echo ! empty( $post_status ) ? esc_attr( $post_status ) : 'all'; ?>" />
			<?php
			submit_button( __( 'Filter', 'aj-woocommerce-wishlist' ), 'button', 'filter_action', false, array( 'id' => 'post-query-submit' ) );

			if ( $need_reset ) {
				echo sprintf( '<a href="%s" class="button button-secondary reset-button">%s</a>', esc_url( add_query_arg( array(
					'page' => 'aj_wcwl_panel',
					'tab'  => 'popular'
				), admin_url( 'admin.php' ) ) ), __( 'Reset', 'aj-woocommerce-wishlist' ) );
			}
		}
	}
}