...
if( ! class_exists( 'AJ_WCWL_Items_Widget' ) ) {
	/**
	 * Wishlist Items Widget
	 *
	 * @since 3.0.0
	 */
	class AJ_WCWL_Items_Widget extends WP_Widget {
		...
		public function __construct(){
			parent::__construct(
				'aj-wcwl-items',
				__( 'AJ Wishlist Items', 'aj-woocommerce-wishlist' ),
				array( 'description' => __( 'A list of products in the user\'s wishlists', 'aj-woocommerce-wishlist' ) )
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$fragments_options = AJ_WCWL_Frontend()->format_fragment_options( $instance, 'AJ_WCWL_Items_Widget' );
			...
			$default_wishlist = AJ_WCWL_Wishlist_Factory::get_default_wishlist();
			$default_wishlist_url = $default_wishlist ? $default_wishlist->get_url() : AJ_WCWL()->get_wishlist_url();
			$multi_wishlist_enabled = AJ_WCWL()->is_multi_wishlist_enabled();
			$wishlists = AJ_WCWL()->get_current_user_wishlists();
			$wishlist_url = ( count( $wishlists ) > 1 && $multi_wishlist_enabled ) ? AJ_WCWL()->get_wishlist_url( 'manage' ) : $default_wishlist_url;
			...
			$additional_info = array(
				...
				'add_all_to_cart_url' => wp_nonce_url( add_query_arg( 'add_all_to_cart', 1, $instance['current_url'] ), 'aj_wcwl_edit_wishlist_action', 'aj_wcwl_edit_wishlist' ),
			...
			aj_wcwl_get_template( 'wishlist-widget-items.php', $args );
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label>
					...
					<?php _e( 'Show extended widget', 'aj-woocommerce-wishlist' ) ?>
				</label><br/>
				<label>
					...
					<?php _e( 'Show mini widget', 'aj-woocommerce-wishlist' ) ?>
				</label>
			</p>
			<p>
				<label for="<?php echo $this->get_field_id( 'show_count' ); ?>">
					...
					<?php _e( 'Show items count', 'aj-woocommerce-wishlist' ); ?>
				</label><br/>
				<label for="<?php echo $this->get_field_id( 'show_default_only' ); ?>">
					...
					<?php _e( 'Show items from default wishlist only', 'aj-woocommerce-wishlist' ); ?>
				</label><br/>
				<label for="<?php echo $this->get_field_id( 'show_add_all_to_cart' ); ?>">
					...
					<?php _e( 'Show Add all to Cart button', 'aj-woocommerce-wishlist' ); ?>
				</label><br/>
				<label for="<?php echo $this->get_field_id( 'show_view_link' ); ?>">
					...
					<?php _e( 'Show View Wishlists link', 'aj-woocommerce-wishlist' ); ?>
				</label>
			</p>
		<?php
		}
