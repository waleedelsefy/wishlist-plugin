...
if ( !class_exists( 'AJ_WCWL_Premium' ) ) {
	/**
	 * WooCommerce Wishlist Premium
	 *
	 * @since 1.0.0
	 */
	class AJ_WCWL_Premium extends AJ_WCWL{
		...
		public function __construct() {
			...
			add_action( 'aj_wcwl_item_is_on_sale', array( $this, 'schedule_on_sale_item_emails' ), 10, 1 );
		}
		 ...
		 * @return \AJ_WCWL_Wishlist|bool First wishlist found where the product occurs (system will privilege default lists)
		 */
		public function get_wishlist_for_product( $product_id ) {
			$items = AJ_WCWL_Wishlist_Factory::get_wishlist_items( array(
			...
			return apply_filters( 'aj_wcwl_wishlist_for_product', $item->get_wishlist(), $product_id );
		}
		 ...
		 * @throws AJ_WCWL_Exception When something goes wrong with creation
		 * @since 3.0.0
		 */
		public function add_wishlist( $atts = array() ) {
			...
			// filtering params
			$wishlist_name = apply_filters( 'aj_wcwl_adding_to_wishlist_wishlist_name', $atts['wishlist_name'] );
			$wishlist_visibility = apply_filters( 'aj_wcwl_adding_to_wishlist_wishlist_visibility', in_array( $atts['wishlist_visibility'], array( 0, 1, 2 ) ) ? $atts['wishlist_visibility'] : 0 );
			$user_id = apply_filters( 'aj_wcwl_adding_to_wishlist_user_id', intval( $atts['user_id'] ) );
			$session_id = apply_filters( 'aj_wcwl_adding_to_wishlist_user_id', $atts['session_id'] );

			if( $wishlist_name == false ){
				throw new AJ_WCWL_Exception( __( 'Wishlist name is required', 'aj-woocommerce-wishlist' ), 0 );
			}
			elseif( strlen( $wishlist_name ) >= 65535 ){
				throw new AJ_WCWL_Exception( __( 'Wishlist name exceeds the maximum number of characters allowed', 'aj-woocommerce-wishlist' ), 0 );
			}
			...
			$new_wishlist = new AJ_WCWL_Wishlist();
		 ...
		 * @return void
		 * @throws AJ_WCWL_Exception When something goes wrong with update
		 * @since 2.0.0
		 */
		public function update_wishlist( $wishlist_id, $args = array() ) {
			...
			if( ! $wishlist ){
				throw new AJ_WCWL_Exception( __( 'Couldn\'t find any wishlist with the provided ID', 'aj-woocommerce-wishlist' ), 0 );
			}

			if( ! $wishlist->current_user_can( 'update_wishlist' ) ){
				throw new AJ_WCWL_Exception(__( 'There was an error while processing your request; please, try later', 'aj-woocommerce-wishlist' ), 0 ); // @since 3.0.7
			}
		 ...
		 *
		 * @param $wishlist_id int
		 * @throws AJ_WCWL_Exception When something goes wrong with deletion
		 * @return void
		 * @since 3.0.0
		 */
		...
		public function filter_woocommerce_template( $core_file, $template, $template_base ) {
			$located = aj_wcwl_locate_template( $template );
		...
		public function add_woocommerce_emails( $emails ) {
			$emails[ 'AJ_WCWL_Estimate_Email' ] = include( AJ_WCWL_INC . 'emails/class.aj-wcwl-estimate-email.php' );
			$emails[ 'AJ_WCWL_Promotion_Email' ] = include( AJ_WCWL_INC . 'emails/class.aj-wcwl-promotion-email.php' );
			$emails[ 'AJ_WCWL_Back_In_Stock_Email' ] = include( AJ_WCWL_INC . 'emails/class.aj-wcwl-back-in-stock-email.php' );
			$emails[ 'AJ_WCWL_On_Sale_Item_Email' ] = include( AJ_WCWL_INC . 'emails/class.aj-wcwl-on-sale-item-email.php' );
		 ...
		 * @return void
		 * @since 1.0
		 * @author Antonio La Rocca <antonio.larocca@ajemes.it>
		 */
		 ...
		 * @see \AJ_WCWL_Form_Handler_Premium::unsubscribe
		 */
		public function get_unsubscribe_link( $user_id ) {
			// retrieve unique unsubscribe token
			$unsubscribe_token = get_user_meta( $user_id, 'aj_wcwl_unsubscribe_token', true );
			...
			if( ! $unsubscribe_token || $unsubscribe_token_expiration < time() ){
				...
				$unsubscribe_token_expiration = apply_filters( 'aj_wcwl_unsubscribe_token_expiration', time() + 30 * DAY_IN_SECONDS, $unsubscribe_token );

				update_user_meta( $user_id, 'aj_wcwl_unsubscribe_token', $unsubscribe_token );
				update_user_meta( $user_id, 'aj_wcwl_unsubscribe_token_expiration', $unsubscribe_token_expiration );
			}

			return apply_filters( 'aj_wcwl_unsubscribe_url', add_query_arg( 'aj_wcwl_unsubscribe', $unsubscribe_token, get_home_url() ), $user_id, $unsubscribe_token, $unsubscribe_token_expiration );
		}
		...
		public function schedule_back_in_stock_emails( $product_id, $stock_status, $product ) {
			...
			$email_options = get_option( 'woocommerce_aj_wcwl_back_in_stock_settings', array() );
			...
			// queue handling
			$queue = get_option( 'aj_wcwl_back_in_stock_queue', array() );
			$unsubscribed = get_option( 'aj_wcwl_unsubscribed_users', array() );
			...
			update_option( 'aj_wcwl_back_in_stock_queue', $queue );
		}
		 ...
		 *
		 * @param $item \AJ_WCWL_Wishlist_Item Item on sale
		 * @return void
		 */
		public function schedule_on_sale_item_emails( $item ) {
			...
			// skip if email ain't active
			$email_options = get_option( 'woocommerce_aj_wcwl_on_sale_item_settings', array() );
			...
			// queue handling
			$queue = get_option( 'aj_wcwl_on_sale_item_queue', array() );
			$unsubscribed = get_option( 'aj_wcwl_unsubscribed_users', array() );
			...
			update_option( 'aj_wcwl_on_sale_item_queue', $queue );
		}
		...
		public function is_multi_wishlist_enabled() {
			$multi_wishlist_enabled = 'yes' == get_option( 'aj_wcwl_multi_wishlist_enable', 'no' );

			if( $multi_wishlist_enabled && ! is_user_logged_in() ){
				$multi_wishlist_enabled = $multi_wishlist_enabled && ( 'yes' == get_option( 'aj_wcwl_enable_multi_wishlist_for_unauthenticated_users', 'no' ) );
			}
		...
		public function get_current_endpoint() {
			...
			return apply_filters( 'aj_wcwl_current_endpoint', $current_endpoint, $action_params );
		}
		...
		public function is_endpoint( $endpoint ) {
			...
			if( 'view' == $endpoint && '' == $current_endpoint && aj_wcwl_is_wishlist_page() ){
				$is_endpoint = true;
			}
			else{
			...
			return apply_filters( 'aj_wcwl_is_endpoint', $is_endpoint, $endpoint, $current_endpoint );
		}
		...
		public function register_plugin_for_activation() {
			if ( ! class_exists( 'DIDO_Plugin_Licence' ) ) {
				require_once( AJ_WCWL_DIR . 'plugin-fw/licence/lib/dido-licence.php' );
				require_once( AJ_WCWL_DIR . 'plugin-fw/licence/lib/dido-plugin-licence.php' );
			}

			DIDO_Plugin_Licence()->register( AJ_WCWL_INIT, AJ_WCWL_SECRET_KEY, AJ_WCWL_SLUG );
		}
		...
		public function register_plugin_for_updates() {
			if ( ! class_exists( 'DIDO_Upgrade' ) ) {
				require_once( AJ_WCWL_DIR . 'plugin-fw/lib/dido-upgrade.php' );
			}

			DIDO_Upgrade()->register( AJ_WCWL_SLUG, AJ_WCWL_INIT );
		}
	}
}

/**
 * Unique access to instance of AJ_WCWL_Premium class
 *
 * @return \AJ_WCWL_Premium
 * @since 2.0.0
 */
function AJ_WCWL_Premium(){
	return AJ_WCWL_Premium::get_instance();
}