...
if ( ! class_exists( 'AJ_WCWL_Session' ) ) {
	/**
	...
	class AJ_WCWL_Session {
		...
		public function set_session_cookie() {
			if ( headers_sent() || ! apply_filters( 'aj_wcwl_set_session_cookie', true ) ) {
				return;
			}
			...
			aj_setcookie( $this->get_session_cookie_name(), $cookie_value, $this->_session_expiration, $this->use_secure_cookie(), true );
		...
		public function get_session_cookie() {
			$cookie_value = aj_getcookie( $this->get_session_cookie_name() ); // @codingStandardsIgnoreLine.
		...
		public function use_secure_cookie() {
			return apply_filters( 'aj_wcwl_session_use_secure_cookie', wc_site_is_https() && is_ssl() );
		}
		...
		public function get_session_cookie_name() {
			if ( empty( $this->_cookie ) ) {
				$this->_cookie = apply_filters( 'aj_wcwl_session_cookie', 'aj_wcwl_session_' . COOKIEHASH );
			}
		...
		public function set_session_expiration() {
			$this->_session_expiring   = time() + aj_wcwl_get_cookie_expiration() - HOUR_IN_SECONDS;
			$this->_session_expiration = time() + aj_wcwl_get_cookie_expiration();
		}
		...
		public function forget_session() {
			aj_destroycookie( $this->get_session_cookie_name() );
...
/**
 * Unique access to instance of AJ_WCWL_Session class
 *
 * @return \AJ_WCWL_Session
 * @since 3.0.0
 */
function AJ_WCWL_Session() {
	return AJ_WCWL_Session::get_instance();
}
