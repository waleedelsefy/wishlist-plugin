...
if ( ! class_exists( 'AJ_WCWL_Ajax_Handler_Premium' ) ) {
	/**
	 * WooCommerce Wishlist Ajax Handler
	 *
	 * @since 3.0.0
	 */
	class AJ_WCWL_Ajax_Handler_Premium {
		/**
		...
		public static function init() {
			...
			add_action( 'wp_ajax_save_privacy', array( 'AJ_WCWL_Ajax_Handler_Premium', 'save_privacy' ) );
			add_action( 'wp_ajax_nopriv_save_privacy', array( 'AJ_WCWL_Ajax_Handler_Premium', 'save_privacy' ) );
			add_action( 'wp_ajax_bulk_add_to_cart', array( 'AJ_WCWL_Ajax_Handler_Premium', 'bulk_add_to_cart' ) );
			add_action( 'wp_ajax_nopriv_bulk_add_to_cart', array( 'AJ_WCWL_Ajax_Handler_Premium', 'bulk_add_to_cart' ) );
			add_action( 'wp_ajax_move_to_another_wishlist', array( 'AJ_WCWL_Ajax_Handler_Premium', 'move_to_another_wishlist' ) );
			add_action( 'wp_ajax_nopriv_move_to_another_wishlist', array( 'AJ_WCWL_Ajax_Handler_Premium', 'move_to_another_wishlist' ) );
			add_action( 'wp_ajax_sort_wishlist_items', array( 'AJ_WCWL_Ajax_Handler_Premium', 'sort_items' ) );
			add_action( 'wp_ajax_nopriv_sort_wishlist_items', array( 'AJ_WCWL_Ajax_Handler_Premium', 'sort_items' ) );
			add_action( 'wp_ajax_update_item_quantity', array( 'AJ_WCWL_Ajax_Handler_Premium', 'update_quantity' ) );
			add_action( 'wp_ajax_nopriv_update_item_quantity', array( 'AJ_WCWL_Ajax_Handler_Premium', 'update_quantity' ) );
			add_action( 'wp_ajax_ask_an_estimate', array( 'AJ_WCWL_Ajax_Handler_Premium', 'ask_an_estimate' ) );
			add_action( 'wp_ajax_nopriv_ask_an_estimate', array( 'AJ_WCWL_Ajax_Handler_Premium', 'ask_an_estimate' ) );
			add_action( 'wp_ajax_remove_from_all_wishlists', array( 'AJ_WCWL_Ajax_Handler_Premium', 'remove_from_all_wishlists' ) );
			add_action( 'wp_ajax_nopriv_remove_from_all_wishlists', array( 'AJ_WCWL_Ajax_Handler_Premium', 'remove_from_all_wishlists' ) );

			// update free responses with premium options.
			add_filter( 'aj_wcwl_ajax_add_return_params', array( 'AJ_WCWL_Ajax_Handler_Premium', 'change_add_return_params' ) );
		}
		...
		public static function save_privacy() {
			...
			if ( ! in_array( $wishlist_privacy, apply_filters( 'aj_wcwl_wishlist_privacy_types', array( 0, 1, 2 ) ) ) ) {
				wp_send_json(
					array(
						'result' => false,
					)
		...
		public static function bulk_add_to_cart() {
			AJ_WCWL_Form_Handler_Premium::bulk_add_to_cart();
		}
		...
		public static function move_to_another_wishlist() {
			...
			if ( $destination_wishlist_token && $origin_wishlist_token && $item_id ) {
				if ( 'new' == $destination_wishlist_token ) {
					try {
						$destination_wishlist = AJ_WCWL()->add_wishlist();
					} catch ( Exception $e ) {
						$destination_wishlist = false;
					}
				}

				$origin_wishlist = AJ_WCWL_Wishlist_Factory::get_wishlist( $origin_wishlist_token );
				$destination_wishlist = isset( $destination_wishlist ) ? $destination_wishlist : AJ_WCWL_Wishlist_Factory::get_wishlist( $destination_wishlist_token );
			...
			if ( $moved ) {
				...
				$message = apply_filters( 'aj_wcwl_moved_element_message', sprintf( __( 'Element correctly moved to %s', 'aj-woocommerce-wishlist' ), $destination_wishlist->get_name() ) );
			}
		...
		public static function ask_an_estimate() {
			...
			try {
				...
				$valid_data             = AJ_WCWL_Form_Handler_Premium::get_valid_additional_data( $_POST, $ask_an_estimate_fields ); // phpcs:ignore WordPress.Security.NonceVerification
			} catch ( Exception $e ) {
				$error = $e->getMessage();
			...
			if ( is_user_logged_in() || $reply_email ) {
				...
				$status = true;
				$message = apply_filters( 'aj_wcwl_estimate_sent', __( 'Estimate request sent', 'aj-woocommerce-wishlist' ) );

				$template_contact_email = apply_filters( 'aj_wcwl_estimate_sent_contact_email', get_option( 'woocommerce_email_from_address' ) );
				$template_icon = apply_filters( 'aj_wcwl_estimate_sent_popup_heading_icon_class', 'fa-envelope-o' );
				$template_heading = apply_filters( 'aj_wcwl_show_popup_heading_icon_instead_of_title', ! empty( $template_icon ), $template_icon ) ? "<i class='fa {$template_icon} heading-icon'></i>" : '';
				$template_title = apply_filters( 'aj_wcwl_estimate_sent_title', __( 'Your request has been sent.<br/>Thanks!', 'aj-woocommerce-wishlist' ) );
				$template_body = apply_filters( 'aj_wcwl_estimate_sent_body', sprintf( '<p class="ask-an-estimate-confirmation">%1$s <a href="mailto:%2$s">%2$s</a></p>', __( 'We will reply to you as soon as possible. For any questions, feel free to contact our customer service at', 'aj-woocommerce-wishlist' ), $template_contact_email ) );

				$template = apply_filters(
					'aj_wcwl_estimate_sent_template',
				...
				$status = false;
				$message = apply_filters( 'aj_wcwl_estimate_missing_email', __( 'You should provide a valid email address that we can use to get back to you', 'aj-woocommerce-wishlist' ) );
			}
		...
		public static function sort_items() {
			...
			$wishlist = aj_wcwl_get_wishlist( $wishlist_token );
		...
		public static function remove_from_all_wishlists() {
			...
			$matching_items = AJ_WCWL_Wishlist_Factory::get_wishlist_items(
				array(
					'product_id' => $prod_id,
		...
		public static function change_add_return_params( $params ) {
			$show_count = get_option( 'aj_wcwl_show_counter' ) == 'yes';

			if ( $show_count && isset( $params['prod_id'] ) ) {
				$params['count'] = aj_wcwl_get_count_text( $params['prod_id'] );
			}
			return $params;
		}
	}
}
AJ_WCWL_Ajax_Handler_Premium::init();
