...
if ( ! class_exists( 'AJ_WCWL_Frontend' ) ) {
	...
	class AJ_WCWL_Frontend {
		...
		/**
		 * Store class aj_WCWL_Install.
		 *
		 * @var object
		 * @access private
		 * @since 1.0.0
		 */
		protected $_aj_wcwl_install;
		...
		public function __construct() {
			define( 'AJ_WCWL_VERSION', $this->version );
			define( 'AJ_WCWL_DB_VERSION', $this->db_version );

			$this->_aj_wcwl_install = AJ_WCWL_Install();

			// add filter for font-awesome compatibility
			add_filter( 'option_aj_wcwl_add_to_wishlist_icon', array( $this, 'update_font_awesome_classes' ) );
			add_filter( 'option_aj_wcwl_add_to_cart_icon', array( $this, 'update_font_awesome_classes' ) );
			...
			add_action( 'aj_wcwl_before_wishlist_title', array( $this, 'print_notices' ) );
			add_action( 'aj_wcwl_wishlist_before_wishlist_content', array( $this, 'wishlist_header' ), 10, 1 );
			add_action( 'aj_wcwl_wishlist_main_wishlist_content', array( $this, 'main_wishlist_content' ), 10, 1 );
			add_action( 'aj_wcwl_wishlist_after_wishlist_content', array( $this, 'wishlist_footer' ), 10, 1 );
			...
			// add AJ WooCommerce Frequently Bought Together Premium shortcode
			add_action( 'aj_wcwl_after_wishlist_form', array( $this, 'aj_wcfbt_shortcode' ), 10, 1 );
			add_filter( 'woocommerce_add_to_cart_redirect', array( $this, 'aj_wfbt_redirect_after_add_to_cart' ), 10, 1 );

			// AJ WCWL Loaded
			do_action( 'aj_wcwl_loaded' );
		}
		...
		public function add_button() {
			$positions = apply_filters( 'aj_wcwl_positions', array(
			...
			$position = get_option( 'aj_wcwl_button_position', 'add-to-cart' );
			...
			$enabled_on_loop = 'yes' == get_option( 'aj_wcwl_show_on_loop', 'no' );

			if( ! $enabled_on_loop ){
				return;
			}

			$positions = apply_filters( 'aj_wcwl_loop_positions', array(
			...
			// Add the link "Add to wishlist"
			$position = get_option( 'aj_wcwl_loop_position', 'after_add_to_cart' );
		...
		public function print_button() {
			 ...
			 * @since 3.0.7
			 */
			if( ! apply_filters( 'aj_wcwl_show_add_to_wishlist', true ) ){
				return;
			}

			echo do_shortcode( "[aj_wcwl_add_to_wishlist]" );
		}
		...
		public function add_nocache_headers() {
			if( ! headers_sent() && aj_wcwl_is_wishlist_page() ){
				wc_nocache_headers();
			}
		}
		...
		public function register_styles() {
			...
			wp_register_style( 'jquery-selectBox', AJ_WCWL_URL . 'assets/css/jquery.selectBox.css', array(), '1.2.0' );
			wp_register_style( 'aj-wcwl-font-awesome', AJ_WCWL_URL . 'assets/css/font-awesome.css', array(), '4.7.0' );
			...
			if( ! $located ){
				wp_register_style( 'aj-wcwl-main', AJ_WCWL_URL . 'assets/css/style.css', array( 'jquery-selectBox', 'aj-wcwl-font-awesome' ), $this->version );
			}
			else{
				...
				wp_register_style( 'aj-wcwl-user-main', $style_url, array( 'jquery-selectBox', 'aj-wcwl-font-awesome' ), $this->version );
			}
			...
			if( $current_theme->exists() ){
				...
				if( file_exists( AJ_WCWL_DIR . 'assets/css/themes/' . $theme_slug . '.css' ) ){
					wp_register_style( 'aj-wcwl-theme', AJ_WCWL_URL . 'assets/css/themes/' . $theme_slug . '.css', array( $located ? 'aj-wcwl-user-main' : 'aj-wcwl-main' ), $this->version );
				}
			}
		}
		...
		public function register_scripts() {
			...
			wp_register_script( 'jquery-selectBox', AJ_WCWL_URL . 'assets/js/jquery.selectBox.min.js', array( 'jquery' ), '1.2.0', true );

			$aj_wcwl_l10n = $this->get_localize();
			$main_script_deps = apply_filters( 'aj_wcwl_main_script_deps', array( 'jquery', 'jquery-selectBox' ) );

			if ( ! $located ) {
				wp_register_script( 'jquery-aj-wcwl', AJ_WCWL_URL . 'assets/js/' . $prefix . 'jquery.aj-wcwl' . $suffix . '.js', $main_script_deps, $this->version, true );
				wp_localize_script( 'jquery-aj-wcwl', 'aj_wcwl_l10n', $aj_wcwl_l10n );
			}
			else {
				wp_register_script( 'jquery-aj-wcwl-user', str_replace( get_stylesheet_directory(), get_stylesheet_directory_uri(), $located ), $main_script_deps, $this->version, true );
				wp_localize_script( 'jquery-aj-wcwl-user', 'aj_wcwl_l10n', $aj_wcwl_l10n );
			}
		}
		...
		public function enqueue_styles_and_stuffs() {
			...
			wp_enqueue_style( 'aj-wcwl-font-awesome' );

			// main plugin style
			if ( ! wp_style_is( 'aj-wcwl-user-main', 'registered' ) ) {
				wp_enqueue_style( 'aj-wcwl-main' );
			}
			else {
				wp_enqueue_style( 'aj-wcwl-user-main' );
			}

			// theme specific style
			if( wp_style_is( 'aj-wcwl-theme', 'registered' ) ){
				wp_enqueue_style( 'aj-wcwl-theme' );;
			}
		...
		public function enqueue_custom_style() {
			...
			if( $custom_css ) {
				$handle = wp_script_is( 'aj-wcwl-user-main' ) ? 'aj-wcwl-user-main' : 'aj-wcwl-main';
		...
		public function enqueue_scripts() {
			...
			if ( ! wp_script_is( 'jquery-aj-wcwl-user', 'registered' ) ) {
				wp_enqueue_script( 'jquery-aj-wcwl' );
			}
			else {
				wp_enqueue_script( 'jquery-aj-wcwl-user' );
			}
		}
		...
		public function get_localize() {
			return apply_filters(
				'aj_wcwl_localize_script',
				array(
					'ajax_url' => admin_url( 'admin-ajax.php', 'relative' ),
					'redirect_to_cart' => get_option( 'aj_wcwl_redirect_cart' ),
					'multi_wishlist' => false,
					'hide_add_button' => apply_filters( 'aj_wcwl_hide_add_button', true ),
					'enable_ajax_loading' => 'yes' == get_option( 'aj_wcwl_ajax_enable', 'no' ),
					'ajax_loader_url' => AJ_WCWL_URL . 'assets/images/ajax-loader-alt.svg',
					'remove_from_wishlist_after_add_to_cart' => get_option( 'aj_wcwl_remove_after_add_to_cart' ) == 'yes',
					'is_wishlist_responsive' => apply_filters( 'aj_wcwl_is_wishlist_responsive', true ),
					'time_to_close_prettyphoto' => apply_filters( 'aj_wcwl_time_to_close_prettyphoto', 3000 ),
					'fragments_index_glue' => apply_filters( 'aj_wcwl_fragments_index_glue', '.' ),
					'reload_on_found_variation' => apply_filters( 'aj_wcwl_reload_on_found_variation', true ),
		...
		public function main_wishlist_content( $var ) {
			...
			aj_wcwl_get_template_part( $template, '', $layout, $var );
		}
		...
		public function wishlist_header( $var ) {
			...
			aj_wcwl_get_template_part( $template, 'header', $layout, $var );
		}
		...
		public function wishlist_footer( $var ) {
			...
			aj_wcwl_get_template_part( $template, 'footer', $layout, $var );
		}
		...
		public function alter_add_to_cart_args( $args ) {
			$use_custom_button = get_option( 'aj_wcwl_add_to_cart_style' );
			$button_class = in_array( $use_custom_button, array( 'button_custom', 'button_default' ) );
			$icon = get_option( 'aj_wcwl_add_to_cart_icon' );
			$custom_icon = get_option( 'aj_wcwl_add_to_cart_custom_icon' );
		...
		public function alter_add_to_cart_text( $text, $product ) {
			$label_option = get_option( 'aj_wcwl_add_to_cart_text', __( 'Add to cart', 'aj-woocommerce-wishlist' ) );
			$label = $product->is_type( 'variable' ) ? $text : apply_filters( 'aj_wcwl_add_to_cart_label', $label_option );
		...
		public function alter_add_to_cart_url( $url, $product ) {
			global $aj_wcwl_wishlist_token;

			if( $aj_wcwl_wishlist_token ){
				$wishlist = aj_wcwl_get_wishlist( $aj_wcwl_wishlist_token );
				...
				if( wp_doing_ajax() ){
					$url = add_query_arg( 'add-to-cart', $product->get_id(), AJ_WCWL()->get_wishlist_url( 'view/' .  $aj_wcwl_wishlist_token ) );
				}

				if( $product->is_type( array( 'simple', 'variation' ) ) && get_option( 'aj_wcwl_redirect_cart' ) == 'yes' ){
					$url = add_query_arg( 'add-to-cart', $product->get_id(), wc_get_cart_url() );
				}

				if( ! $product->is_type( 'external' ) && get_option( 'aj_wcwl_remove_after_add_to_cart' ) == 'yes' ){
					$url = add_query_arg(
						array(
							'remove_from_wishlist_after_add_to_cart' => $product->get_id(),
							'wishlist_id' => $wishlist_id,
							'wishlist_token' => $aj_wcwl_wishlist_token
						),
						$url
					);
				}

				if( $item && 'yes' == get_option( 'aj_wcwl_quantity_show' ) ){
					$url = add_query_arg( 'quantity', $item->get_quantity(), $url );
				}
			}

			return apply_filters( 'dido_wcwl_add_to_cart_redirect_url', esc_url_raw( $url ), $url, $product );
		}
		 ...
		 */
		public function aj_wcfbt_shortcode( $meta ){

			if( ! ( defined( 'AJ_WFBT' ) && AJ_WFBT ) || get_option( 'aj_wfbt_enable_integration' ) == 'no' ) {
				return;
			}

			$products = AJ_WCWL()->get_products(
				array(
			...
			do_shortcode( '[aj_wfbt products="' . implode( ',', $ids ) . '"]' );
		}

		/**
		 * Redirect after add to cart from AJ WooCommerce Frequently Bought Together Premium shortcode
		 *
		 * @since 2.0.0
		 */
		public function aj_wfbt_redirect_after_add_to_cart( $url ){
			if( ! isset( $_REQUEST['aj_wfbt_shortcode'] ) ) {
				return $url;
			}

			return get_option( 'aj_wcwl_redirect_cart' ) == 'yes' ? wc_get_cart_url() : AJ_WCWL()->get_wishlist_url();
		}
		...
		protected function _build_custom_css( $rules = array() ){
			$generated_code = '';
			$rules = apply_filters( 'aj_wcwl_custom_css_rules', array_merge( array(
				'color_add_to_wishlist' => array(
					...
					'deps' => array(
						'aj_wcwl_add_to_wishlist_style' => 'button_custom'
					)
				),
				...
				'color_add_to_cart' => array(
					...
					'deps' => array(
						'aj_wcwl_add_to_cart_style' => 'button_custom'
					)
				),
				...
				'color_button_style_1' => array(
					'selector' => '.woocommerce .hidden-title-form button,
								   .aj-wcwl-wishlist-new .create-wishlist-button,
								   .wishlist_manage_table tfoot button.submit-wishlist-changes,
								   .aj-wcwl-wishlist-search-form button.wishlist-search-button',
					'rules'    => array(
						'background' => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#333333'
				...
				'color_headers_background' => array(
					'selector' => '.wishlist_table thead tr th,
								   ...
								   .widget_aj-wcwl-lists ul.dropdown li.current a,
								   .widget_aj-wcwl-lists ul.dropdown li a:hover,
				...
				'color_share_button' => array(
					...
					'deps' => array(
						'aj_wcwl_enable_share' => 'yes'
					)
				),
				'color_fb_button' => array(
					'selector' => '.aj-wcwl-share a.facebook',
					'rules'    => array(
						'background'  => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#39599E'
					...
					'deps' => array(
						'aj_wcwl_enable_share' => 'yes',
						'aj_wcwl_share_fb' => 'yes',
					)
				),
				'color_tw_button' => array(
					'selector' => '.aj-wcwl-share a.twitter',
					'rules'    => array(
						'background'  => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#45AFE2'
					...
					'deps' => array(
						'aj_wcwl_enable_share' => 'yes',
						'aj_wcwl_share_twitter' => 'yes',
					)
				),
				'color_pr_button' => array(
					'selector' => '.aj-wcwl-share a.pinterest',
					'rules'    => array(
						'background'  => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#AB2E31'
					...
					'deps' => array(
						'aj_wcwl_enable_share' => 'yes',
						'aj_wcwl_share_pinterest' => 'yes',
					)
				),
				'color_em_button' => array(
					'selector' => '.aj-wcwl-share a.email',
					'rules'    => array(
						'background'  => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#FBB102'
					...
					'deps' => array(
						'aj_wcwl_enable_share' => 'yes',
						'aj_wcwl_share_email' => 'yes',
					)
				),
				'color_wa_button' => array(
					'selector' => '.aj-wcwl-share a.whatsapp',
					'rules'    => array(
						'background'  => array(
							'rule'    => 'background: %1$s; background-color: %1$s;',
							'default' => '#00A901'
			...
			if( $custom_css = get_option( 'aj_wcwl_custom_css' ) ){
		...
		protected function _update_cookies(){
			$cookie = aj_getcookie( 'aj_wcwl_products' );
			$new_cookie = array();
			...
			if( ! empty( $cookie ) ) {
				...
				aj_setcookie( 'aj_wcwl_products', $new_cookie );
			}
		}
		...
		protected function _convert_cookies_to_session(){
			...
			if( ! empty( $cookie ) ){
				...
				$default_list = AJ_WCWL_Wishlist_Factory::get_default_wishlist();
				...
				foreach ( $cookie as $item ){
					...
					$new_item = new AJ_WCWL_Wishlist_Item();
				...
				aj_destroycookie( 'aj_wcwl_products' );
            }
		}
	}
}
 ...
 *
 * @return \AJ_WCWL_Frontend|\AJ_WCWL_Frontend_Premium
 * @since 2.0.0
 */
function AJ_WCWL_Frontend(){
	return defined( 'AJ_WCWL_PREMIUM' ) ? AJ_WCWL_Frontend_Premium::get_instance() : AJ_WCWL_Frontend::get_instance();
}
