...
<tr class="<?php echo esc_attr( $info['TextDomain'] ); ?>">
	<td class="product-name">
		<span class="show-if-is-mobile field-name"><?php esc_html_e( 'License:', 'aj-plugin-upgrade-fw' ); ?></span>
		<?php echo esc_html( $this->display_product_name( $info['Name'] ) ); ?>
	</td>
	<td class="product-licence-email">
		<span class="show-if-is-mobile field-name"><?php esc_html_e( 'Email:', 'aj-plugin-upgrade-fw' ); ?></span>
		<?php
		$license_email      = '';
	...
	<td class="product-licence-key">
		<span class="show-if-is-mobile field-name"><?php esc_html_e( 'License Key:', 'aj-plugin-upgrade-fw' ); ?></span>
		<?php echo esc_html( sprintf( '%s-****-****-****-************', substr( $info['licence']['licence_key'], 0, 8 ) ) ); ?>
	</td>
	<td class="product-licence-remaining">
		<span class="show-if-is-mobile field-name"><?php esc_html_e( 'Licenses used:', 'aj-plugin-upgrade-fw' ); ?></span>
		<?php
		$license_remaining = sprintf(
			/* translators: %1$1s: Number of activations for the licence. %2$2s: The activations number limit */
			esc_html__( '%1$1s out of %2$2s', 'aj-plugin-upgrade-fw' ),
	...
	if ( ! empty( $info['licence']['status_code'] ) && 200 !== intval( $info['licence']['status_code'] ) ) {
		...
		switch ( $info['licence']['status_code'] ) {
			case 106:
				$error_message = esc_html_x( 'expired', 'License status', 'aj-plugin-upgrade-fw' );
				break;
			case 107:
				$error_message = esc_html_x( 'banned', 'License status', 'aj-plugin-upgrade-fw' );
				break;
			default:
	...
	<td class="product-licence-expire-on <?php echo esc_attr( $error_message ); ?>">
		<span class="show-if-is-mobile field-name"><?php esc_html_e( 'Expires on:', 'aj-plugin-upgrade-fw' ); ?></span>
		<div class="aj-license-expire-on-wrapper <?php echo esc_attr( $error_message ); ?>">
			<?php
			if ( ! empty( $info['licence']['status_code'] ) && 200 !== intval( $info['licence']['status_code'] ) ) {
				printf( '<span class="aj-license-status-message %s">%s</span>', $error_message, $error_message ); //@codingStandardsIgnoreLine

				if ( 106 === intval( $info['licence']['status_code'] ) ) {
					// Expired.
					$buy_again = ! empty( $info['PluginURI'] ) ? $info['PluginURI'] : 'https://ajemes.com';
					printf(
						'<a href="%s" class="aj-renew-expired-license button-primary" target="_blank"  rel="nofollow noopener">%s</a>',
						esc_html( $renew_url ),
						esc_html_x( 'renew', 'button label', 'aj-plugin-upgrade-fw' )
					); //@codingStandardsIgnoreLine

					printf(
						'<a href="%s" class="aj-buy-again-license" target="_blank"  rel="nofollow noopener">%s &gt;</a>',
						esc_html( $buy_again ),
						esc_html_x( 'Or get a new license', 'this is a short text for a link', 'aj-plugin-upgrade-fw' )
					); //@codingStandardsIgnoreLine
				}
			} else {
	...
	<td class="product-deactivate-button">
		...
		<?php $button_action = $is_expired_status_code ? 'aj_remove-' . $this->get_product_type() : 'aj_deactivate-' . $this->get_product_type(); ?>
		<input
				type="button"
				value="<?php echo esc_html_x( 'Remove', 'Button label', 'aj-plugin-upgrade-fw' ); ?>"
