...
if ( ! class_exists( 'AJ_Plugin_Upgrade' ) ) {
	/**
	 * DIDO Upgrade
	 * Notify and Update plugin
	 *
	 * @class       AJ_Plugin_Upgrade
	 * @since       1.0
	 * @author      AJ
	 * @package     AJ
	...
	class AJ_Plugin_Upgrade {
		 ...
		 * @var string
		 */
		protected $remote_url = 'https://update.ajemes.com/plugin-xml.php';
		 ...
		 * @var string
		 */
		protected $package_url = 'https://licence.ajemes.com/api/download/';
		...
		public function __construct() {
			...
			if ( defined( 'DIDO_LICENCE_DEBUG' ) && DIDO_LICENCE_DEBUG ) {
				$this->package_url = defined( 'DIDO_LICENCE_UPGRADE_LOCALHOST' ) ? DIDO_LICENCE_UPGRADE_LOCALHOST : 'https://staging-licenceajemes.kinsta.cloud/api/download/';
				add_filter( 'block_local_requests', '__return_false' );
			}

			add_action( 'wp_ajax_aj_plugin_fw_get_premium_changelog', array( $this, 'show_changelog_for_premium_plugins' ) );
			...
			if ( isset( $_GET['plugin'] ) && isset( $_GET['section'] ) && 'changelog' === $_GET['section'] ) {
				...
				if ( isset( $this->plugins[ $plugin_init ] ) ) {
					...
					if ( ! empty( $this->plugins[ $plugin_init ]['info']['changelog'] ) ) {
						...
						$template    = DIDO_CORE_PLUGIN_TEMPLATE_PATH . '/upgrade/changelog.php';
						if ( file_exists( $template ) ) {
							include $template;
						}
						die();
					}
					$error    = esc_html__( 'An unexpected error occurred, please try again later. Thanks!', 'aj-plugin-upgrade-fw' );
					$template = DIDO_CORE_PLUGIN_TEMPLATE_PATH . '/upgrade/error.php';
					if ( file_exists( $template ) ) {
						include $template;
					} else {
		...
		public function register( $plugin_slug, $plugin_init ) {
			...
			$transient                     = 'aj_register_' . md5( $plugin_slug );
			if ( apply_filters( 'aj_register_delete_transient', false ) ) {
				delete_transient( $transient );
			}
			$info = get_transient( $transient );
			if ( false === $info || apply_filters( 'aj_register_delete_transient', false ) ) {
		...
		public function admin_enqueue_scripts() {
			...
			if ( 'plugins.php' === $pagenow && defined( 'DIDO_CORE_PLUGIN_URL' ) ) {
				$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
				wp_enqueue_style( 'dido-upgrader', DIDO_CORE_PLUGIN_URL . '/assets/css/dido-upgrader.css' );
				wp_enqueue_script( 'aj-update-plugins', DIDO_CORE_PLUGIN_URL . '/assets/js/aj-update-plugins' . $suffix . '.js', array( 'jquery' ), false, true );
				...
				$update_plugins_localized = array(
					...
					'l10n'       => array(
						...
						'updating' => _x( 'Updating %s...', 'plugin-fw', 'aj-plugin-upgrade-fw' ), // No ellipsis.
						/* translators: %s: Plugin name and version */
						'updated'  => _x( '%s updated!', 'plugin-fw', 'aj-plugin-upgrade-fw' ),
						/* translators: %s: Plugin name and version */
						'failed'   => _x( '%s update failed', 'plugin-fw', 'aj-plugin-upgrade-fw' ),
					),
				);

				wp_localize_script( 'aj-update-plugins', 'aj_plugin_fw', $update_plugins_localized );
			}
		}
		...
		public function get_plugin_upgrading( $upgrader ) {
			...
			if ( empty( $plugin ) ) {
				...
				if( $is_wp_cli || $is_manageWP ){
					$plugins = AJ_Plugin_Licence()->get_products();
			...
			return $plugin ? AJ_Plugin_Licence()->get_product( $plugin ) : array();
		}
		...
		public function upgrader_pre_download( $reply, $package, $upgrader ) {
			...
			// If class AJ_Plugin_Licence doesn't exists or plugin upgrading is not an AJ one, return.
			...
			$licence    = AJ_Plugin_Licence()->get_licence();
			$product_id = $plugin['product_id'];

			if ( empty( $licence[ $product_id ] ) ) {
				return new WP_Error( 'license_not_valid', esc_html_x( 'You have to activate the plugin to benefit from automatic updates.', '[Update Plugin Message: License not enabled]', 'aj-plugin-upgrade-fw' ) );
			}
			...
			$args = array(
				...
				'instance'    => AJ_Plugin_Licence()->get_home_url(),
			);
			...
			$upgrader->skin->feedback( 'downloading_package', esc_html__( 'AJ Repository', 'aj-plugin-upgrade-fw' ) );

			$download_file = $this->download_url( $package, $args );

			// Regenerate update_plugins transient.
			aj_plugin_fw_force_regenerate_plugin_update_transient();
		...
		protected function download_url( $url, $body, $timeout = 300 ) {
			...
			if ( ! $url ) {
				return new WP_Error( 'http_no_url', esc_html__( 'Invalid URL Provided.', 'aj-plugin-upgrade-fw' ) );
			}
			...
			if ( ! $tmpfname ) {
				return new WP_Error( 'http_no_file', esc_html__( 'Could not create Temporary file.', 'aj-plugin-upgrade-fw' ) );
			}
			...
			// Firstly we check if ajemes gives a 404 error. In this case the upgrade won't check on backup system.
			if ( 404 === $response_code ) {
				...
				$url = add_query_arg( $body, 'https://casper.ajemes.com' );
				unset( $args['body'] );
		...
		public function check_update( $transient, $save = false ) {
			...
			foreach ( $this->plugins as $init => $plugin ) {
				...
				$is_activated   = AJ_Plugin_Licence()->check( $init );

				$item = array(
					'id'                => $init,
					'plugin'            => $init,
					'aj_premium_slug' => $plugin_slug,
		...
		public function avoid_auto_update_bulk( $value, $old_value, $option, $network_id ) {
			...
			$value = array_filter( $value, function( $p ) {
				return empty( $this->plugins[ $p ] ) || ( is_plugin_active( $p ) && AJ_Plugin_Licence()->check( $p ) );
			});
		...
		public function plugin_update_row() {
			...
			$update_now_class = apply_filters( 'aj_plugin_fw_update_now_class', '' );
			$update_now_class = trim( $update_now_class . ' aj-update-link update-link' );
			...
			$details_url 	= admin_url( 'admin-ajax.php?action=aj_plugin_fw_get_premium_changelog&tab=plugin-information&plugin=' . $init . '&section=changelog&TB_iframe=true&width=640&height=662' );

			// If is a multisite and cause the licence ar for blog check if for current blog the licence is active.
			$is_active     = is_multisite() ? AJ_Plugin_Licence()->check( $init ) : ! ! $r->package;
			...
			if ( ! current_user_can( 'update_plugins' ) ) {
				...
				printf( __( 'There is a new version of %1$s available. <a href="%2$s" class="thickbox dido-changelog-button open-plugin-details-modal" title="%3$s">View version %4$s details</a>.', 'aj-plugin-upgrade-fw' ), $this->plugins[ $init ]['info']['Name'], esc_url( $details_url ), esc_attr( $this->plugins[ $init ]['info']['Name'] ), $r->new_version );
			} elseif ( is_plugin_active_for_network( $init ) || is_network_admin() ) {
				// translators: %1$s, %3$s are placeholders for the plugin name, %2$s the link to open changelog modal, %4$s is the new plugin version.
				printf( __( 'There is a new version of %1$s available. <a href="%2$s" class="thickbox dido-changelog-button open-plugin-details-modal" title="%3$s">View version %4$s details</a>. <em>You have to activate the plugin on a single site of the network to benefit from automatic updates.</em>', 'aj-plugin-upgrade-fw' ), $this->plugins[ $init ]['info']['Name'], esc_url( $details_url ), esc_attr( $this->plugins[ $init ]['info']['Name'] ), $r->new_version );
			} elseif ( ! $is_active ) {
				...
				printf( __( 'There is a new version of %1$s available. <a href="%2$s" class="thickbox dido-changelog-button open-plugin-details-modal" title="%3$s">View version %4$s details</a>. <em>Automatic update is unavailable for this plugin, please <a href="%5$s" title="License activation">activate</a> your copy of %1$s.</em>', 'aj-plugin-upgrade-fw' ), $this->plugins[ $init ]['info']['Name'], esc_url( $details_url ), esc_attr( $this->plugins[ $init ]['info']['Name'] ), $r->new_version, AJ_Plugin_Licence()->get_licence_activation_page_url() );
			} else {
				printf( __( 'There is a new version of %1$s available. <a href="%2$s" class="thickbox dido-changelog-button open-plugin-details-modal" title="%3$s">View version %4$s details</a> or <a href="%5$s" class="%6$s" data-plugin="%7$s" data-slug="%8$s" data-name="%1$s">update now</a>.', 'aj-plugin-upgrade-fw' ), $this->plugins[ $init ]['info']['Name'], esc_url( $details_url ), esc_attr( $this->plugins[ $init ]['info']['Name'] ), $r->new_version, wp_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' ) . $init, 'upgrade-plugin_' . $init ), $update_now_class, $init, $this->plugins[ $init ]['slug'] );
			}

			if ( version_compare( $this->plugins[ $init ]['info']['Version'], $r->new_version, '>' ) ) {
				printf( __( '<br/><b>Please note:</b> You are using a higher version than the latest available one. </em>Please, make sure you\'ve downloaded the latest version of <em>%1$s</em> from the only <a href="https://ajemes.com" target="_blank">AJ official website</a>, specifically, from your <a href="https://ajemes.com/my-account/recent-downloads/" target="_blank">Downloads page</a>. This is the only way to be sure the version you are using is 100%% malware-free.', 'aj-plugin-upgrade-fw' ), $this->plugins[ $init ]['info']['Name'], esc_url( $details_url ), esc_attr( $this->plugins[ $init ]['info']['Name'] ), $r->new_version, AJ_Plugin_Licence()->get_licence_activation_page_url(), $this->plugins[ $init ]['info']['Name'] );
			}
		...
		public function in_theme_update_message( $plugin, $changelog, $changelog_id, $echo = true ) {
			$res = "<div id='{$changelog_id}' class='dido-plugin-changelog-wrapper'>
                    <div class='dido-plugin-changelog'>
                        <h2 class='dido-plugin-changelog-title'>{$plugin['info']['Name']} - Changelog</h2>
                        <p>{$changelog}</p>
                    </div>
                </div>";
		...
		public function get_remote_url( $plugin_info ) {
			...
			if ( function_exists( 'AJ_Plugin_Licence' ) && false !== $slug ) {
				// Get license for AJ Plugins.
				$enabled_license = AJ_Plugin_Licence()->get_licence();
			...
			$args = array(
                'plugin'                => $slug,
                'instance'              => function_exists('AJ_Plugin_Licence') ? md5( AJ_Plugin_Licence()->get_home_url() ) : md5( $_SERVER['SERVER_NAME'] ),
                'license'               => $license,
			...
			$args = apply_filters( 'aj_get_remove_url_args', $args );
...
if ( ! function_exists( 'AJ_Plugin_Upgrade' ) ) {
	 ...
	 * @return AJ_Plugin_Upgrade
	 */
	function AJ_Plugin_Upgrade() { // phpcs:ignore
		return AJ_Plugin_Upgrade::instance();
	}
}
