...
if ( ! class_exists( 'AJ_Licence' ) ) {
	...
	abstract class AJ_Licence {
		 ...
		 * @var string
		 */
		protected $licence_option = 'dido_products_licence_activation';

		/**
		 * The ajemes licence api uri
		 *
		 * @since 1.0
		 * @var string
		 */
		protected $api_uri = 'https://licence.ajemes.com/api/';

		/**
		 * The ajemes api uri query args
		 *
		 * @since 1.0
		 * @var string
		 */
		...
		public function __construct() {
			...
			$is_debug_enabled = defined( 'DIDO_LICENCE_DEBUG' ) && DIDO_LICENCE_DEBUG;
			if ( $is_debug_enabled ) {
				$this->api_uri = defined( 'DIDO_LICENCE_DEBUG_LOCALHOST' ) ? DIDO_LICENCE_DEBUG_LOCALHOST : 'https://staging-licenceajemes.kinsta.cloud/api/';
			...
			load_textdomain( 'aj-plugin-upgrade-fw', get_template_directory() . '/core/plugin-upgrade/languages/aj-plugin-upgrade-fw-' . apply_filters( 'plugin_locale', get_locale(), 'aj-plugin-upgrade-fw' ) . '.mo' )

			// ...or from plugin folder
			|| load_textdomain( 'aj-plugin-upgrade-fw', dirname( __DIR__ ) . '/languages/aj-plugin-upgrade-fw-' . apply_filters( 'plugin_locale', get_locale(), 'aj-plugin-upgrade-fw' ) . '.mo' );

			if ( function_exists( 'aj_plugin_fw_add_requirements' ) ) {
				aj_plugin_fw_add_requirements( __( 'AJ License Activation', 'aj-plugin-fw' ), array( 'min_tls_version' => '1.2' ) );
			}

			add_action( 'wp_ajax_aj_license_banner_dismiss', array( $this, 'dismiss_license_banner' ) );
		}
		...
		public function admin_enqueue_scripts() {
			...
			if ( false !== strpos( $current_plugin_upgrade_path, 'wp-content/themes' ) ) {
				...
				$style_path  = get_template_directory_uri() . '/theme/plugins/dido-framework/plugin-upgrade/';
				$script_path = $style_path;
			}

			wp_register_script( 'dido-license-utils', $script_path . 'assets/js/dido-license-utils' . $suffix . '.js', array( 'jquery' ), $this->version, true );
			wp_register_script( 'dido-licence', $script_path . 'assets/js/dido-licence' . $suffix . '.js', array( 'jquery', 'wc-enhanced-select' ), $this->version, true );
			wp_register_style( 'dido-theme-licence', $style_path . 'assets/css/dido-licence.css', array(), $this->version );
			wp_register_style( 'aj-license-banner', $style_path . 'assets/css/aj-license-banner.css', array(), $this->version );
			...
			wp_localize_script(
				...
				'licence_message',
				array(
					'error'                      => sprintf( esc_html_x( 'Please, insert a valid %s', '%s = field name', 'aj-plugin-upgrade-fw' ), '%field%' ),
					'errors'                     => sprintf( esc_html__( 'Please, insert a valid %s and a valid %s', 'aj-plugin-upgrade-fw' ), '%field_1%', '%field_2%' ),
					'server'                     => esc_html__( 'Unable to contact remote server: this occurs when there are issues with connecting to our own servers. Trying again after a few minutes should solve the issue. If the problem persists please submit a support ticket and we will be happy to help you.', 'aj-plugin-upgrade-fw' ),
					'email'                      => esc_html__( 'email address', 'aj-plugin-upgrade-fw' ),
					'license_key'                => esc_html__( 'license key', 'aj-plugin-upgrade-fw' ),
					'are_you_sure'               => esc_html__( 'Are you sure you want to deactivate the license for current site?', 'aj-plugin-upgrade-fw' ),
					'choose_the_plugin_singular' => esc_html( _nx( 'Plugin', 'Choose the plugin', 1, 'Form Label', 'aj-plugin-upgrade-fw' ) ),
					'choose_the_plugin_plural'   => esc_html( _nx( 'Plugin', 'Choose the plugin', 2, 'Form Label', 'aj-plugin-upgrade-fw' ) ),
				)
			);

			wp_localize_script( 'dido-licence', 'script_info', array( 'is_debug' => defined( 'DIDO_LICENCE_DEBUG' ) && DIDO_LICENCE_DEBUG ) );
			wp_localize_script( 'dido-licence', 'aj_ajax', array( 'url' => admin_url( 'admin-ajax.php', 'relative' ) ) );
			wp_localize_script( 'dido-license-utils', 'aj_ajax', array( 'url' => admin_url( 'admin-ajax.php', 'relative' ) ) );

			/* Enqueue Scripts only in Licence Activation page of plugins and themes */
			if ( strpos( get_current_screen()->id, 'aj_plugins_activation' ) !== false || strpos( get_current_screen()->id, 'dido_panel_license' ) !== false ) {
				wp_enqueue_script( 'dido-licence' );
				wp_enqueue_style( 'dido-theme-licence' );
			...
			if ( $this->show_license_banner() ) {
				...
				wp_enqueue_script( 'dido-license-utils' );
				wp_enqueue_style( 'aj-license-banner' );
			}
		}
			...
			if ( $product ) {
				...
				if ( $body && is_array( $body ) && isset( $body['activated'] ) && $body['activated'] ) {
					$license = array(
						...
						'marketplace'          => isset( $body['marketplace'] ) ? $body['marketplace'] : 'aj',
					);
		...
		public function update_licence_information() {
			...
			do_action( 'dido_licence_after_check' );
		...
		public function get_response_code_message( $code, $args = array() ) {
			...
			$messages = array(
				'100'         => __( 'Invalid Request', 'aj-plugin-upgrade-fw' ),
				'101'         => __( 'Invalid license key', 'aj-plugin-upgrade-fw' ),
				'102'         => __( 'Software has been deactivated', 'aj-plugin-upgrade-fw' ),
				'103'         => __( 'Maximum number of license activations reached', 'aj-plugin-upgrade-fw' ),
				'104'         => __( 'Invalid instance ID', 'aj-plugin-upgrade-fw' ),
				'105'         => __( 'Invalid security key', 'aj-plugin-upgrade-fw' ),
				'106'         => __( 'License key has expired', 'aj-plugin-upgrade-fw' ),
				'107'         => __( 'License key has been revoked', 'aj-plugin-upgrade-fw' ),
				'108'         => __( 'This product is not included in your AJ Club Subscription Plan', 'aj-plugin-upgrade-fw' ),
				'200'         => sprintf( '<strong>%s</strong>! %s', __( 'Great', 'aj-plugin-upgrade-fw' ), __( 'License successfully activated', 'aj-plugin-upgrade-fw' ) ),
				'deactivated' => sprintf( '%s <strong>%s</strong>', __( 'License key deactivated for website', 'aj-plugin-upgrade-fw' ), isset( $instance ) ? $instance : '' ),
		...
		/**
		 * Do requests to ajemes licence network
		 *
		...
		protected function do_request( $url, $body = array(), $method = 'POST' ) {
			...
			$request_args = apply_filters( 'aj_plugin_fw_do_request_args', $request_args );
			$response     = wp_remote_request( $url, $request_args );

			if ( is_wp_error( $response ) || ( isset( $response['response'] ) && isset( $response['response']['code'] ) && floor( $response['response']['code'] / 100 ) >= 4 ) ) {
				$alternative_url = esc_url_raw( add_query_arg( $body, 'https://casper.ajemes.com/wc-api/software-api/' ) );
				...
				if ( ! ! $products_to_activate ) {
					...
					if ( ! ! $product_names ) {
						...
						foreach ( $product_names as $id => $name ){
							$product_list_items .= sprintf( '<span class="aj-license-product-item"><a href="%s">%s</a></span>', $this->get_license_activation_url( $id ), $name );
						}
						$product_list 	= '<div class="aj-license-products-list">'. $product_list_items . '</div>';
						$activation_url = $this->get_license_url();
						$nonce          = wp_create_nonce( 'dismiss-aj-license-banner' );
						?>
						<div id="aj-license-notice" class="notice notice-error is-dismissible" data-nonce="<?php echo esc_attr( $nonce ); ?>">
							<?php $img_logo = plugin_dir_url( __DIR__ ) . 'assets/images/logo-aj.svg'; ?>
							<div class="aj-license-logo-wrapper">
								<img class="aj-license-logo" src="<?php echo $img_logo; ?>" />
							</div>
							<div class="aj-license-notice-message">
								<?php $warning = esc_html__( 'Thank you for choosing AJ to improve your shop!', 'aj-plugin-upgrade-fw' ); ?>
								<?php $message = esc_html_x( "Please %s to get premium features and support for:", '%s is a placeholder for a link "set your license key"', 'aj-plugin-upgrade-fw' ) ?>
								<?php $url = sprintf( '<a href="%s">%s</a>', $activation_url, esc_html__( 'set your license key', 'aj-plugin-upgrade-fw' ) ); ?>
								<?php $message = sprintf( $message, $url ); ?>
								<?php printf( "<strong>%s</strong> %s %s", $warning, $message, $product_list ); ?>
							</div>
							<div class="aj-license-set-license-button">
								<?php printf( '<a class="button button-primary" href="%s">%s</a>', $activation_url, esc_html_x( 'set your licenses', 'Button label','aj-plugin-upgrade-fw' ) ); ?>
							</div>
						</div>
						<?php
					}
		...
		protected function show_license_banner() {
			$show = get_user_meta( get_current_user_id(), 'aj-license-banner', true );
		...
		public function dismiss_license_banner() {
			...
			if ( $nonce && wp_verify_nonce( $nonce, 'dismiss-aj-license-banner' ) ) {
				update_user_meta( get_current_user_id(), 'aj-license-banner', 'hide' );
			}
		}
	}
}
