...
( function ( $ ) {
    ...
    $.aj_license_animation = function( elem, spinner, action ){
        var table_wrapper           = $( '#licence-check-section-wrapper' ),
            no_license_activated    = $( '#no-license-enabled-message' ),
            no_license_to_enable    = $( '#aj-no-license-to-enabled-message' ),
            plugins_form            = $('#aj-license-from-wrapper');
        ...
        if( action == 'beforeSend' ){
            ...
            if( $( '#aj-products-list' ).find( 'option' ).length === 0 ){
                plugins_form.fadeIn();
            ...
            if( $( '#aj-products-list' ).find( 'option' ).length === 1 ){
    ...
    var licence_activation = function ( button ) {
        button.off('click').on( 'click', function ( e, button ) {
            ...
            var t                       = $( this ),
                form                    = $('#aj-license-activation'),
                products_list           = $( '#aj-products-list' ),
                ...
                message_wrapper         = $( '#aj-licence-notice-wrapper' ),
                message                 = $( '#aj-licence-notice' ),
                data                    = null,
            ...
            if ( error === false ) {
                jQuery.ajax({
                    ...
                    data: data,
                    beforeSend: function () {
                        $.aj_license_animation( form, spinner, 'beforeSend' );
                    },
                    success: function (response) {
                        $.aj_license_animation( form, spinner, 'afterSend' );

                        if (true === response.activated) {
                            var table = $('#aj-enabled-license'),
                            ...
                            $('html, body').animate({
                                ...
                                if( product_count == 1 || product_count == 0 ){
                                    ...
                                    $('.aj-select-plugin').text( licence_message.choose_the_plugin_singular );
                                }

                                else {
                                    products_list.attr( 'disabled', false );
                                    $('.aj-select-plugin').text( licence_message.choose_the_plugin_plural );
                                }
                            });

                            $.aj_license_animation( new_elem, spinner, 'addLine' );
                            message.find('p.aj-licence-notice-message').html('#' + response.code + ': ' + response.activation_message);
    ...
    var licence_update = function (button) {
        button.off('click').on('click', function (e) {
            ...
            var t = $(this),
                ...
                license_table = $('#aj-enabled-license'),
                data = form.serialize();
            ...
            jQuery.ajax({
                ...
                data: data,
                beforeSend: function () {
                    $.aj_license_animation( license_table, form.find('div.spinner'), 'beforeSend' );
                },
                success: function (response) {
        ...
        button.off('click').on( 'click', function ( e ) {
            ...
            if ( check == true ) {
                ...
                jQuery.ajax({
                    ...
                    beforeSend: function () {
                        $.aj_license_animation( tr_to_remove, spinner, 'beforeSend' );
                    },
                    success: function (response) {


                        if (false == response) {
                            message.find('p.aj-licence-notice-message').html(licence_message.server);
                            message_wrapper.fadeIn();
                        }

                        else {
                            if (false == response.activated) {
                                $.aj_license_animation( tr_to_remove, spinner, 'removeLine' );
                                add_option_to_product_list(t, $( '#aj-products-list' ) );
                                product_count = form.data( 'count' );
                                form.removeClass( 'count-' + product_count );
                                product_count = ( product_count * 1 ) + 1;
                                form.data( 'count', product_count ).addClass( 'count-' + product_count );
                            ...
                            if (typeof response.error != 'undefined') {
                                message.find('p.aj-licence-notice-message').html(response.error);
                                message_wrapper.fadeIn();
                            }
                        }
                    }
    ...
    var remove_license_expired = function ( button ){
        button.off('click').on( 'click', function ( e ) {
            ...
            if( check ){
                var t            = $(this),
                    bubble       = $('#aj-expired-license-count'),
                ...
                jQuery.ajax({
                        ...
                        success: function ( response ) {
                            $.aj_license_animation( tr_to_remove, spinner, response.code == 999 ? 'removeLine' : 'afterSend' );
    ...
    $body.on( 'click', '.dido-changelog-button', function ( e ) {
        $( '#TB_window' ).remove();
    } );
