	...
	<table class="shop_table cart wishlist_table wishlist_manage_table responsive" cellspacing="0">
		...
		<tr>
			<th class="wishlist-name">
				<span class="nobr">
					<?php echo esc_html( apply_filters( 'aj_wcwl_wishlist_manage_name_heading', __( 'Wishlists', 'aj-woocommerce-wishlist' ) ) ); ?>
				</span>
			</th>

			<th class="wishlist-privacy">
				<span class="nobr">
					<?php echo esc_html( apply_filters( 'aj_wcwl_wishlist_manage_privacy_heading', __( 'Privacy', 'aj-woocommerce-wishlist' ) ) ); ?>
				</span>
			</th>
			...
			?>
				<th class="wishlist-item-count">
					<span class="nobr">
						<?php echo esc_html( apply_filters( 'aj_wcwl_wishlist_manage_number_of_items_heading', __( 'Count of items', 'aj-woocommerce-wishlist' ) ) ); ?>
					</span>
				</th>
			<?php endif; ?>
			...
			?>
				<th class="wishlist-dateadded">
					<span class="nobr">
						<?php echo esc_html( apply_filters( 'aj_wcwl_wishlist_manage_dateadded_heading', __( 'Created on', 'aj-woocommerce-wishlist' ) ) ); ?>
					</span>
				</th>
			<?php endif; ?>
			...
			?>
				<th class="wishlist-download">
					<span class="nobr">
						<?php echo esc_html( apply_filters( 'aj_wcwl_wishlist_manage_download_heading', __( 'PDF Download', 'aj-woocommerce-wishlist' ) ) ); ?>
					</span>
				</th>
			<?php endif; ?>
			...
			?>
				<th class="wishlist-delete">
					<span class="nobr">
						<?php echo esc_html( apply_filters( 'aj_wcwl_wishlist_manage_delete_heading', __( 'Delete', 'aj-woocommerce-wishlist' ) ) ); ?>
					</span>
				</th>
			<?php endif; ?>
		</tr>
			...
			<tr data-wishlist-id="<?php echo esc_attr( $wishlist->get_id() ); ?>">
				...
				<td class="wishlist-privacy">
					<select name="wishlist_options[<?php echo esc_attr( $wishlist->get_id() ); ?>][wishlist_privacy]" class="wishlist-visibility selectBox">
						<option value="0" class="public-visibility" <?php selected( $wishlist->get_privacy(), 0 ); ?> ><?php echo aj_wcwl_get_privacy_label( 0 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></option>
						<option value="1" class="shared-visibility" <?php selected( $wishlist->get_privacy(), 1 ); ?> ><?php echo aj_wcwl_get_privacy_label( 1 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></option>
						<option value="2" class="private-visibility" <?php selected( $wishlist->get_privacy(), 2 ); ?> ><?php echo aj_wcwl_get_privacy_label( 2 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></option>
					</select>
				</td>
				...
				<?php if ( $show_number_of_items ): ?>
					<td class="wishlist-item-count">
						...
						echo esc_html( sprintf( __( '%d items', 'aj-woocommerce-wishlist' ), $wishlist->count_items() ) );
						?>
					</td>
				<?php endif; ?>
				...
				<?php if ( $show_delete_wishlist ): ?>
					<td class="wishlist-delete">
						<a class="button wishlist-delete" onclick="return confirm('<?php esc_html_e( 'Are you sure you want to delete this wishlist?', 'aj-woocommerce-wishlist' ); ?>');" href="<?php echo esc_url( $wishlist->get_delete_url() ); ?>"><?php esc_html_e( 'Delete', 'aj-woocommerce-wishlist' ); ?></a>
					</td>
				<?php endif; ?>
			</tr>
...
<?php else: ?>
	<p class="wishlist-empty">
		<?php echo wp_kses_post( AJ_WCWL_Frontend_Premium()->get_no_wishlist_message() ); ?>
	</p>
<?php endif; ?>
