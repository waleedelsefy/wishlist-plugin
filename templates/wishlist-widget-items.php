 ...
 * @var $default_wishlist       AJ_WCWL_Wishlist Default list
...
<?php endif; ?>
	...
	<div class="content <?php echo esc_attr( $instance['style'] ); ?> aj-wcwl-items-<?php echo esc_html( $instance['unique_id'] ); ?> woocommerce wishlist-fragment on-first-load" data-fragment-options="<?php echo esc_attr( json_encode( $fragments_options ) ); ?>">
		...
		<div class="heading">
			<div class="items-counter">
				...
				<a href="<?php echo esc_url( $wishlist_url ); ?>">
					...
					<span class="heading-icon">
						<?php echo apply_filters( 'aj_wcwl_widget_items_extended_heading_icon', $heading_icon ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</span>
			...
			<?php if ( isset( $instance['style'] ) && 'extended' == $instance['style'] ) : ?>
				<h3 class="heading-title"><?php echo esc_html( apply_filters( 'aj_wcwl_widget_items_extended_title', __( 'Wishlist', 'aj-woocommerce-wishlist' ) ) ); ?></h3>
			<?php endif; ?>
		</div>

		<div class="list">
			<?php if ( ! $instance['ajax_loading'] && isset( $instance['show_count'] ) && 'yes' === $instance['show_count'] && 'mini' === $instance['style'] && count( $products ) ) : ?>
				<p class="items-count"><?php echo esc_html( sprintf( __( '%d items in wishlist', 'aj-woocommerce-wishlist' ), count( $products ) ) ); ?></p>
			<?php endif; ?>
							...
							<div class="mini-cart-item-info">
								...
								<?php if ( 'yes' != $instance['show_default_only'] ) : ?>
									<small class="mini-cart-wishlist-info">
										<?php printf( '%s: %s', esc_html__( 'In', 'aj-woocommerce-wishlist' ), implode( ', ', $info['in_list'] ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
									</small>
								<?php endif; ?>
							</div>
						</li>
			...
			<?php else: ?>
				<p class="empty-wishlist">
					<?php echo esc_html( apply_filters( 'aj_wcwl_widget_items_empty_list', __( 'Please, add your first item to the wishlist', 'aj-woocommerce-wishlist' ) ) ); ?>
				</p>
			<?php endif; ?>

			<?php if ( count( $products ) && 'yes' === $instance['show_view_link'] ) : ?>
				<a class="show-wishlist" href="<?php echo esc_url( $wishlist_url ); ?> "><?php esc_html_e( 'View your wishlist &rsaquo;', 'aj-woocommerce-wishlist' ); ?></a>
			<?php endif; ?>

			<?php if ( count( $products ) && 'yes' === $instance['show_add_all_to_cart'] ) : ?>
				<a class="btn button alt add_all_to_cart" href="<?php echo esc_url( $add_all_to_cart_url ); ?>"><?php esc_html_e( 'Add all to cart', 'aj-woocommerce-wishlist' ); ?></a>
			<?php endif; ?>
		</div>
