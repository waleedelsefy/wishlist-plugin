...
<div id="ask_an_estimate_popup">
	<form action="<?php echo esc_url( $ask_estimate_url ); ?>" method="post" class="wishlist-ask-an-estimate-popup">
		<div class="aj-wcwl-popup-content">
			<?php if ( apply_filters( 'aj_wcwl_show_popup_heading_icon_instead_of_title', true, 'fa-envelope-open-o' ) ) : ?>
				<i class="fa <?php echo esc_attr( apply_filters( 'aj_wcwl_ask_and_estimate_popup_heading_icon_class', 'fa-envelope-open-o' ) ); ?> heading-icon"></i>
			<?php else : ?>
				<h3><?php esc_html_e( 'Ask for an estimate for this list', 'aj-woocommerce-wishlist' ); ?></h3>
			<?php endif; ?>

			<p class="popup-description">
				<?php echo esc_html( apply_filters( 'aj_wcwl_ask_for_an_estimate_popup_text', __( 'Ask for an estimate for this list:', 'aj-woocommerce-wishlist' ) ) ); ?>
			</p>

			<?php if ( ! is_user_logged_in() ) : ?>
				<p class="form-row form-row-wide">
					<label for="reply_email"><?php echo esc_html( apply_filters( 'aj_wcwl_ask_estimate_reply_mail_label', __( 'Your email', 'aj-woocommerce-wishlist' ) ) ); ?></label>
