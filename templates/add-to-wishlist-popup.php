...
<div class="aj-wcwl-add-button">
	...
	<a href="#add_to_wishlist_popup_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>" rel="nofollow" class="<?php echo esc_attr( $link_classes ); ?> open-pretty-photo" data-rel="prettyPhoto[add_to_wishlist_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>]" data-title="<?php echo esc_attr( apply_filters( 'aj_wcwl_add_to_wishlist_title', $label ) ); ?>">
	...
	<div id="add_to_wishlist_popup_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>" class="aj-wcwl-popup">
		<form class="aj-wcwl-popup-form" method="post" action="<?php echo esc_url( add_query_arg( 'add_to_wishlist', $product_id, $base_url ) ); ?>">
			<div class="aj-wcwl-popup-content">
				...
				<p class="popup-description">
					<a href="#" class="show-tab active" data-tab="choose"><?php esc_html_e( 'Choose a wishlist', 'aj-woocommerce-wishlist' ); ?></a>
					<?php esc_html_e( 'or', 'aj-woocommerce-wishlist' ); ?>
					<a href="#" class="show-tab" data-tab="create"><?php esc_html_e( 'Create a new list', 'aj-woocommerce-wishlist' ); ?></a>
				</p>
					...
					<div class="aj-wcwl-wishlist-select-container">
						<p class="form-row form-row-wide">
							<select name="wishlist_id" class="wishlist-select">
								<?php if ( empty( $lists ) ): ?>
									<option value="0" <?php selected( true ); ?> ><?php echo esc_html( apply_filters( 'aj_wcwl_default_wishlist_name', get_option( 'aj_wcwl_wishlist_title' ) ) ); ?></option>
								<?php else: ?>
				...
				<div class="tab create">
					<p class="form-row form-row-wide">
						<input name="wishlist_name" class="wishlist-name input-text" type="text" placeholder="<?php echo esc_html( apply_filters( 'aj_wcwl_new_list_title_text', __( 'Enter wishlist name', 'aj-woocommerce-wishlist' ) ) ); ?>"/>
					</p>
