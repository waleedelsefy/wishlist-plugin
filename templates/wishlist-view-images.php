...
<!-- WISHLIST GRID -->
<ul
	class="wishlist_table wishlist_view shop_table cart images_grid responsive products columns-<?php echo esc_attr( apply_filters( 'aj_wcwl_wishlist_view_images_columns', wc_get_loop_prop( 'columns' ) ) ); ?> <?php echo $no_interactions ? 'no-interactions' : ''; ?> <?php echo $enable_drag_n_drop ? 'sortable' : ''; ?>"
	...
	if ( $wishlist && $wishlist->has_items() ) :
		foreach ( $wishlist_items as $item ) :
			...
			if ( $product && $product->exists() ) :
				?>
				<li id="aj-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" <?php wc_product_class( '', $product ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					<div class="item-wrapper">
						...
						<?php do_action( 'aj_wcwl_wishlist_view_images_after_thumbnail', $item ); ?>
						<div class="item-details">
							<div class="item-details-wrapper">
								<a class="close" href="#" title="<?php esc_html_e( 'Close', 'aj-woocommerce-wishlist' ); ?>">&times;</a>
								...
								<?php do_action( 'aj_wcwl_table_after_product_name', $item ); ?>
								...
								<?php if ( $show_variation || $show_dateadded || $show_price || $show_quantity || $show_stock_status ) : ?>
									<table class="item-details-table">
										...
										<?php if ( $show_dateadded && $item->get_date_added() ) : ?>
											<tr class="date-added">
												<td class="label">
													<?php esc_html_e( 'Added on:', 'aj-woocommerce-wishlist' ); ?>
												</td>
												<td class="value">
										...
										<?php if ( $show_price || $show_price_variations ) : ?>
											<tr class="product-price">
												<td class="label">
													<?php esc_html_e( 'Price:', 'aj-woocommerce-wishlist' ); ?>
												</td>
												<td class="value">
													<?php
													if ( $show_price ) {
										...
										<?php if ( $show_quantity ) : ?>
											<tr class="product-quantity">
												<td class="label">
													<?php esc_html_e( 'Quantity:', 'aj-woocommerce-wishlist' ); ?>
												</td>
												<td class="value">
										...
										<?php if ( $show_stock_status ) : ?>
											<tr class="product-stock-status">
												<td class="label">
													<?php esc_html_e( 'Stock:', 'aj-woocommerce-wishlist' ); ?>
												</td>
												<td class="value">
													<?php echo 'out-of-stock' === $stock_status ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of stock', 'aj-woocommerce-wishlist' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'aj-woocommerce-wishlist' ) . '</span>'; ?>
												</td>
											</tr>
										<?php endif; ?>
								...
								<?php if ( $show_remove_product ) : ?>
									<div class="product-remove">
										<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove_from_wishlist" title="<?php echo esc_html( apply_filters( 'aj_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'aj-woocommerce-wishlist' ) ) ); ?>"><i class="fa fa-trash"></i></a>
									</div>
								<?php endif; ?>
		...
		<li class="wishlist-empty">
			<?php echo esc_html( apply_filters( 'aj_wcwl_no_product_to_remove_message', __( 'No products added to the wishlist', 'aj-woocommerce-wishlist' ) ) ); ?>
		</li>
	<?php endif; ?>
</ul>
