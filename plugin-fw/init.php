<?php
/**
 * Framework Name: DIDO Plugin Framework
 * Version: 3.5.6
 * Author: AJ
 * Text Domain: aj-plugin-fw
...
if ( ! function_exists( 'dido_maybe_plugin_fw_loader' ) ) {
	/**
	 * Load the framework if it's not yet loaded.
	 *
	 * @param string $plugin_path The plugin path.
	 */
	function dido_maybe_plugin_fw_loader( $plugin_path ) {
		...
		$plugin_fw_main_file = $plugin_path . 'plugin-fw/dido-plugin.php';
		...
		if ( file_exists( $plugin_upgrade_path ) ) {
			if ( ! empty( $plugin_upgrade_fw_data ) ) {
				foreach ( $plugin_upgrade_fw_data as $version => $files ) {
					if ( version_compare( $version, $framework_data['Version'], '<' ) ) {
						$plugin_upgrade_fw_data = array( $framework_data['Version'] => dido_get_upgrade_files( $plugin_license_path, $plugin_upgrade_path ) );
					}
				}
			} else {
				$plugin_upgrade_fw_data = array( $framework_data['Version'] => dido_get_upgrade_files( $plugin_license_path, $plugin_upgrade_path ) );
...
if ( ! function_exists( 'dido_get_upgrade_files' ) ) {
	/**
	 ...
	 */
	function dido_get_upgrade_files( $plugin_license_path, $plugin_upgrade_path = '' ) {
		$to_include = array();

		if ( ! ! $plugin_license_path ) {
			$plugin_upgrade_path = empty( $plugin_upgrade_path ) ? $plugin_license_path : $plugin_upgrade_path;
			$license_files       = array(
				'%aj-license-path%/lib/dido-licence.php',
				'%aj-license-path%/lib/dido-plugin-licence.php',
				'%aj-license-path%/lib/dido-theme-licence.php',
			);

			$upgrade_files = array( '%aj-upgrade-path%/lib/dido-plugin-upgrade.php' );

			$to_include_license = str_replace( '%aj-license-path%', $plugin_license_path, $license_files );
			$to_include_upgrade = str_replace( '%aj-upgrade-path%', $plugin_upgrade_path, $upgrade_files );
