...
$extra_row_classes = $is_required ? array( 'aj-plugin-fw--required' ) : array();
$extra_row_classes = (array) apply_filters( 'aj_plugin_fw_panel_wc_extra_row_classes', $extra_row_classes, $field );

$row_classes = array( 'aj-plugin-fw-panel-wc-row', $field['type'] );
$row_classes = array_merge( $row_classes, $extra_row_classes, array( $extra_row_class ) );
$row_classes = implode( ' ', $row_classes );

?>
<tr valign="top" class="<?php echo esc_attr( $row_classes ); ?>" <?php echo aj_field_deps_data( $field ); ?>>
	<?php if ( $display_row ) : ?>
		...
		<td class="forminp forminp-<?php echo esc_attr( $field['type'] ); ?>">
			<?php aj_plugin_fw_get_field( $field, true ); ?>
