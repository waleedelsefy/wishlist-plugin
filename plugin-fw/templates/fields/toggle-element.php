...
list ( $field_id, $class, $name, $value, $add_button, $elements, $the_title, $subtitle, $onoff_field, $sortable, $save_button, $delete_button, $custom_attributes ) = aj_plugin_fw_extract( $field, 'id', 'class', 'name', 'value', 'add_button', 'elements', 'title', 'subtitle', 'onoff_field', 'sortable', 'save_button', 'delete_button', 'custom_attributes' );
...
?>
<div class="aj-toggle_wrapper <?php echo esc_attr( $class_wrapper ); ?>" id="<?php echo esc_attr( $field_id ); ?>" data-nonce="<?php echo esc_attr( $ajax_nonce ); ?>">
	<?php if ( ! empty( $label ) ) : ?>
		<label for="<?php esc_attr( $field_id ); ?>"><?php echo esc_html( $label ); ?></label>
	<?php endif; ?>
	<?php if ( $show_add_button ) : ?>
		<button class="aj-add-button aj-add-box-button"
		...
		<div id="<?php echo esc_attr( $field_id ); ?>_add_box" class="aj-add-box"></div>
		<script type="text/template" id="tmpl-aj-toggle-element-add-box-content-<?php echo esc_attr( $field_id ); ?>">
			<?php foreach ( $elements as $element ) : ?>
				<?php
				$element['title'] = $element['name'];
				$element['type']  = isset( $element['aj-type'] ) ? $element['aj-type'] : $element['type'];

				unset( $element['aj-type'] );
				...
				if ( ! empty( $element['required'] ) ) {
					$class_element .= ' aj-plugin-fw--required';
				}
				?>
				<div class="aj-add-box-row aj-toggle-content-row <?php echo esc_attr( $class_element ); ?> <?php echo '{{{data.index}}}'; ?>" <?php echo aj_field_deps_data( $element ); ?>>
			...
			<?php if ( ! empty( $save_button ) ) : ?>
				<div class="aj-add-box-buttons">
					<button class="button-primary aj-save-button">
	...
	<div class="aj-toggle-elements">
		<?php if ( $values ) : ?>
			<?php foreach ( $values as $i => $value ) : ?>
				<?php
				$title_element    = aj_format_toggle_title( $the_title, $value );
				$title_element    = apply_filters( 'aj_plugin_fw_toggle_element_title_' . $field_id, $title_element, $elements, $value );
				$subtitle_element = aj_format_toggle_title( $subtitle, $value );
				$subtitle_element = apply_filters( 'aj_plugin_fw_toggle_element_subtitle_' . $field_id, $subtitle_element, $elements, $value );
				?>
				<div id="<?php echo esc_attr( $field_id ); ?>_<?php echo esc_attr( $i ); ?>"
						class="aj-toggle-row <?php echo ! empty( $subtitle ) ? 'with-subtitle' : ''; ?> <?php echo esc_attr( $class ); ?>"
					...
					<div class="aj-toggle-title">
						...
						<?php if ( ! empty( $onoff_field ) && is_array( $onoff_field ) ) : ?>
							...
							<span class="aj-toggle-onoff"
								...
								<?php endif ?>
							>
								<?php aj_plugin_fw_get_field( $onoff_field, true ); ?>
							</span>

							<?php if ( $sortable ) : ?>
								<span class="aj-icon aj-icon-drag"></span>
							<?php endif ?>
						<?php endif; ?>
					</div>
					<div class="aj-toggle-content">
						<?php if ( $elements && count( $elements ) > 0 ) : ?>
							<?php foreach ( $elements as $element ) : ?>
								...
								if ( ! empty( $element['required'] ) ) {
									$element['class_row'] .= ' aj-plugin-fw--required';
								}
								?>
								<div class="aj-toggle-content-row <?php echo esc_attr( $element['class_row'] . ' ' . $element['type'] ); ?>" <?php echo aj_field_deps_data( $element ); ?>>
									<label for="<?php echo esc_attr( $element['id'] ); ?>"><?php echo esc_html( $element['title'] ); ?></label>
									<div class="aj-plugin-fw-option-with-description">
										<?php aj_plugin_fw_get_field( $element, true ); ?>
										<span class="description"><?php echo ! empty( $element['desc'] ) ? wp_kses_post( $element['desc'] ) : ''; ?></span>
							...
							<?php endforeach; ?>
						<?php endif; ?>
						<div class="aj-toggle-content-buttons">
							...
							<?php if ( $save_button && ! empty( $save_button['id'] ) ) : ?>
								...
								?>
								<button id="<?php echo esc_attr( $save_button['id'] ); ?>" class="button-primary aj-save-button <?php echo esc_attr( $save_button_class ); ?>">
							...
							<?php if ( $delete_button && ! empty( $delete_button['id'] ) ) : ?>
								...
								<button id="<?php echo esc_attr( $delete_button['id'] ); ?>"
										class="button-secondary aj-delete-button <?php echo esc_attr( $delete_button_class ); ?>">
	...
	<script type="text/template" id="tmpl-aj-toggle-element-item-<?php echo esc_attr( $field_id ); ?>">
		<div id="<?php echo esc_attr( $field_id ); ?>_{{{data.index}}}"
				class="aj-toggle-row highlight <?php echo ! empty( $subtitle ) ? 'with-subtitle' : ''; ?> <?php echo esc_attr( $class ); ?>"
		...
		>
			<div class="aj-toggle-title">
				...
				<span class="aj-toggle"><span class="aj-icon aj-icon-arrow_right"></span></span>
				<?php if ( ! empty( $onoff_field ) && is_array( $onoff_field ) ) : ?>
					<?php
				...
				<?php endif; ?>
				<?php if ( $sortable ) : ?>
					<span class="aj-icon aj-icon-drag ui-sortable-handle"></span>
				<?php endif ?>
			</div>
			<div class="aj-toggle-content">
				<?php if ( $elements && count( $elements ) > 0 ) : ?>
					<?php foreach ( $elements as $element ) : ?>
						...
						?>
						<div class="aj-toggle-content-row <?php echo esc_attr( $class_element . ' ' . $element['type'] ); ?>" <?php echo aj_field_deps_data( $element ); ?>>
							<label for="<?php echo esc_attr( $element['id'] ); ?>"><?php echo esc_html( $element['title'] ); ?></label>
							<div class="aj-plugin-fw-option-with-description">
								<?php aj_plugin_fw_get_field( $element, true ); ?>
								<span class="description"><?php echo ! empty( $element['desc'] ) ? wp_kses_post( $element['desc'] ) : ''; ?></span>
				...
				<div class="aj-toggle-content-buttons">
					...
					<?php if ( $save_button && ! empty( $save_button['id'] ) ) : ?>
						...
						$save_button_name  = isset( $save_button['name'] ) ? $save_button['name'] : '';
						?>
						<button id="<?php echo esc_attr( $save_button['id'] ); ?>" class="aj-save-button <?php echo esc_attr( $save_button_class ); ?>">
							<?php echo esc_html( $save_button_name ); ?>
						</button>
					<?php endif; ?>
					<?php if ( $delete_button && ! empty( $delete_button['id'] ) ) : ?>
						...
						$delete_button_name  = isset( $delete_button['name'] ) ? $delete_button['name'] : '';
						?>
						<button id="<?php echo esc_attr( $delete_button['id'] ); ?>" class="button-secondary aj-delete-button <?php echo esc_attr( $delete_button_class ); ?>">
							<?php echo esc_html( $delete_button_name ); ?>
						</button>
					<?php endif; ?>
				</div>
