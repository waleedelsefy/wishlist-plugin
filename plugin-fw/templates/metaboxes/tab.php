...
global $post;
$classes  = apply_filters( 'aj_plugin_fw_metabox_class', $class, $post );
$classes  = aj_plugin_fw_remove_duplicate_classes( $classes );
$ul_style = count( $tabs ) <= 1 ? 'display:none;' : '';
$i        = 0;
do_action( 'dido_before_metaboxes_tab' );

?>
	<div class="aj-plugin-fw metaboxes-tab <?php echo esc_attr( $classes ); ?>">
		<?php do_action( 'dido_before_metaboxes_labels' ); ?>
		<ul class="metaboxes-tabs clearfix" style="<?php echo esc_attr( $ul_style ); ?>">
			<?php foreach ( $tabs as $key => $_tab ) : ?>
				...
				$anchor_id = 'aj-plugin-fw-metabox-tab-' . urldecode( $key ) . '-anchor';
				...
				$i ++;
				?>
				<li id="<?php echo esc_attr( $anchor_id ); ?>" class="<?php echo esc_attr( $class ); ?>" <?php echo aj_field_deps_data( $_tab ); ?>>
		...
		<?php do_action( 'dido_after_metaboxes_labels' ); ?>

		<?php
		if ( isset( $_tab['label'] ) ) {
			do_action( 'dido_before_metabox_option_' . urldecode( $key ) );
		}
		?>

		<?php wp_nonce_field( 'metaboxes-fields-nonce', 'dido_metaboxes_nonce' ); ?>
		...
		<?php foreach ( $tabs as $key => $_tab ) : ?>
			<div class="tabs-panel" id="<?php echo esc_attr( urldecode( $key ) ); ?>">
				...
				$_tab['fields'] = apply_filters( 'dido_metabox_' . $key . '_tab_fields', $_tab['fields'] );
				?>

				<?php foreach ( $_tab['fields'] as $id_tab => $field ) : ?>
					<?php
					$field_name = $field['name'];
					$field_name = str_replace( 'dido_metaboxes[', '', $field_name );
					$pos        = strpos( $field_name, ']' );
					if ( $pos ) {
						$field_name = substr_replace( $field_name, '', $pos, 1 );
					}
					$value                  = dido_get_post_meta( $post->ID, $field_name );
					...
					<div class="<?php echo esc_attr( $container_classes ); ?>">
						<?php
						$field_template_path = aj_plugin_fw_get_field_template_path( $field );
						if ( $field_template_path ) {
							$display_row                 = 'hidden' !== $field['type'];
							$display_row                 = isset( $field['aj-display-row'] ) ? ! ! $field['aj-display-row'] : $display_row;
							$field['display-field-only'] = in_array( $field['type'], array( 'hidden', 'html', 'sep', 'simple-text', 'title', 'list-table' ), true );

							if ( $display_row ) {

								$field_row_path = apply_filters( 'aj_plugin_fw_metabox_field_row_template_path', DIDO_CORE_PLUGIN_TEMPLATE_PATH . '/metaboxes/field-row.php', $field );
							...
							$args       = apply_filters(
								'dido_fw_metaboxes_type_args',
								array(
									'basename' => DIDO_CORE_PLUGIN_PATH,
							...
							dido_plugin_get_template( $basename, $field_path . $field_type . '.php', $field_args );
						}
						?>
					</div>
				<?php endforeach ?>
...
do_action( 'dido_after_metaboxes_tab' );
