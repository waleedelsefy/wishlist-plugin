...
( function ( $ ) {
	...
	$( act_page_option ).insertAfter( '#dido-post-setting .handlediv' );
	$( act_page_option ).insertAfter( '#dido-page-setting .handlediv' );


	$( '#_active_page_options-container' ).on( 'click', function () {
		if ( $( '#_active_page_options' ).is( ":checked" ) ) {
			$( '#dido-page-setting .inside .metaboxes-tab, #dido-post-setting .inside .metaboxes-tab' ).css( {
																											   'opacity'       : 1,
																											   'pointer-events': 'auto'
																										   } );
		} else {
	...
	$( document.body ).on( 'aj-plugin-fw-metabox-init-deps', function () {
		$( document.body ).trigger( 'aj-plugin-fw-init-radio' );
		$( '.metaboxes-tab [data-dep-target]:not(.aj-plugin-fw-metabox-deps-initialized)' ).each( function () {
			var t = $( this );
			...
			t.addClass( 'aj-plugin-fw-metabox-deps-initialized' );
		} );
	} ).trigger( 'aj-plugin-fw-metabox-init-deps' );
