(function ($) {
    ...
    const aj_icon = el('svg', {width: 22, height: 22},
    ...
    function create_shortcode(sc_args, props, callback) {
        ...
        if (typeof props.callback != 'undefined' && typeof $[props.callback] == 'function') {
            ...
            gutenberg_preview = '<span class="aj_block_' + block_id + '">' + sc + '</span>';

            if (callback == 'edit' && props.do_shortcode != false) {
                do_shortcode = (function (block_id) {
                    var ajax_call_date = null;
                    $(document).trigger( 'aj_plugin_fw_gutenberg_before_do_shortcode', [sc, block_id] );
                    $.ajax({
                        async: true,
                        url: aj_gutenberg_ajax.ajaxurl,
                        method: 'post',
                        data: {action: 'aj_plugin_fw_gutenberg_do_shortcode', shortcode: sc},
                        success: function (data) {
                            ajax_call_date = data;
                            if (ajax_call_date != '') {
                                $('.aj_block_' + block_id).html(ajax_call_date);
                                $(document).trigger( 'aj_plugin_fw_gutenberg_success_do_shortcode', [sc, block_id, ajax_call_date] );
                            }
                        }
                    });
                    $(document).trigger( 'aj_plugin_fw_gutenberg_after_do_shortcode', [sc, block_id, ajax_call_date] );
    ...
    $.each(aj_gutenberg, function ($block, $props) {
        registerBlockType("aj/" + $block, {
            title: $props.title,
            description: $props.description,
            category: $props.category,
            attributes: $props.attributes,
            icon: typeof $props.icon != 'undefined' ? $props.icon : aj_icon,
