...
jQuery( function ( $ ) {
	...
	function dependencies_handler( id, deps, values, type ) {
		...
		var $current_field     = $( id ),
			$current_container = $( id + '-container' ).closest( 'tr' ); // container for DIDO Plugin Panel

		if ( $current_container.length < 1 ) {
			// container for DIDO Plugin Panel WooCommerce
			$current_container = $current_field.closest( '.aj-plugin-fw-panel-wc-row, .aj-toggle-content-row' );
		}
		...
		for ( j in types ) {
			...
			if ( !result ) {
				switch ( current_type ) {
					case 'disable':
						$current_container.addClass( 'aj-disabled' );
				...
				switch ( current_type ) {
					case 'disable':
						$current_container.removeClass( 'aj-disabled' );
	...
	function init_dependencies() {
		$( '[data-dep-target]:not( .deps-initialized )' ).each( function () {
			...
			if ( field_type === 'select-images' ) {
				event = 'aj_select_images_value_changed';
			}
	...
	$( document ).on( 'aj-add-box-button-toggle', init_dependencies );
	...
	$( '.aj-plugin-fw-panel .woo-nav-tab-wrapper' ).removeClass( 'woo-nav-tab-wrapper' ).addClass( 'aj-nav-tab-wrapper' );

	var wrap    = $( '.wrap.aj-plugin-ui' ).first(),
	...
	var active_subnav = $( document ).find( '.aj-nav-sub-tab.nav-tab-active' );

	if ( active_subnav.length ) {
		// WP page
		var mainWrapper = $( document ).find( '.aj-plugin-fw-wp-page-wrapper' );
		if ( !mainWrapper.length ) {
			mainWrapper = $( document ).find( '#wpbody-content > .aj-plugin-ui' );
		}

		if ( mainWrapper ) {
			// serach first for deafult wrap
			var wrap = mainWrapper.find( '.dido-admin-panel-content-wrap' );
			if ( wrap.length ) {
