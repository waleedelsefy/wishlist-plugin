...
if ( ! class_exists( 'DIDO_Metabox' ) ) {
	...
	class DIDO_Metabox {
		 ...
		 *
		 * @return DIDO_Metabox
		 * @author Antonino Scarfi' <antonino.scarfi@ajemes.com>
		 */
		...
		public function init( $options = array() ) {
			...
			add_filter( 'dido_icons_screen_ids', array( $this, 'add_screen_ids_for_icons' ) );

			add_action( 'wp_ajax_aj_plugin_fw_save_toggle_element_metabox', array( $this, 'save_toggle_element' ) );
		}
		...
		public function enqueue() {
			...
			$enqueue = apply_filters( 'aj_plugin_fw_metabox_enqueue_styles_and_scripts', $enqueue, $this );
			...
			if ( $enqueue ) {
				...
				wp_enqueue_style( 'dido-plugin-metaboxes' );
				wp_enqueue_style( 'jquery-ui-style' );

				wp_enqueue_script( 'dido-metabox' );
		...
		public function get_option_metabox_name( $field_id, $private = true ) {
			$db_name = apply_filters( 'dido_metaboxes_option_main_name', 'dido_metaboxes' );
			$return  = $db_name . '[';
		...
		public function show() {
			...
			dido_plugin_get_template( DIDO_CORE_PLUGIN_PATH, 'metaboxes/tab.php', $args );
		}
		...
		public function save_postdata( $post_id ) {
			if ( ! isset( $_POST['dido_metaboxes_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['dido_metaboxes_nonce'] ), 'metaboxes-fields-nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				return $post_id;
			}

			$allow_ajax = isset( $_REQUEST['aj_metabox_allow_ajax_saving'] ) && sanitize_key( wp_unslash( $_REQUEST['aj_metabox_allow_ajax_saving'] ) ) === $this->id;
			...
			if ( isset( $_POST['dido_metaboxes'] ) ) {
				$dido_metabox_data = wp_unslash( $_POST['dido_metaboxes'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

				if ( is_array( $dido_metabox_data ) ) {
					foreach ( $dido_metabox_data as $field_name => $field_value ) {
		...
		public function sanitize_and_save_fields( $post_id ) {
			...
			$allow_ajax              = isset( $_REQUEST['aj_metabox_allow_ajax_saving'] ) && sanitize_key( wp_unslash( $_REQUEST['aj_metabox_allow_ajax_saving'] ) ) === $this->id;
			$ajax_partial_saving_tab = isset( $_REQUEST['aj_metabox_allow_ajax_partial_saving_tab'] ) ? sanitize_key( wp_unslash( $_REQUEST['aj_metabox_allow_ajax_partial_saving_tab'] ) ) : false;
		...
		public function sanitize_and_save_field( $field, $post_id ) {
			...
			$meta_box_data = isset( $_POST['dido_metaboxes'] ) ? wp_unslash( $_POST['dido_metaboxes'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			if ( isset( $meta_box_data[ $field['id'] ] ) ) {
				if ( in_array( $field['type'], array( 'onoff', 'checkbox' ), true ) ) {
						...
						if ( $elements_value ) {
							...
							foreach ( $field['elements'] as $element ) {
								foreach ( $elements_value as $key => $element_value ) {
									...
									if ( ! empty( $element['aj-sanitize-callback'] ) && is_callable( $element['aj-sanitize-callback'] ) ) {
										$elements_value[ $key ][ $element['id'] ] = call_user_func( $element['aj-sanitize-callback'], $elements_value[ $key ][ $element['id'] ] );
									}
								}
							}
						}
					...
					if ( ! empty( $field['aj-sanitize-callback'] ) && is_callable( $field['aj-sanitize-callback'] ) ) {
						$value = call_user_func( $field['aj-sanitize-callback'], $value );
					}
		...
		public function save_toggle_element() {
			...
			if ( ! isset( $_REQUEST['dido_metaboxes_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_REQUEST['dido_metaboxes_nonce'] ), 'metaboxes-fields-nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				return;
			}
			...
			if ( isset( $_REQUEST['dido_metaboxes'], $_REQUEST['toggle_id'], $_REQUEST['metabox_tab'], $_REQUEST['dido_metaboxes'][ $_REQUEST['toggle_id'] ] ) ) {
				$meta_box_data = isset( $_REQUEST['dido_metaboxes'] ) ? wp_unslash( $_REQUEST['dido_metaboxes'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
if ( ! function_exists( 'dido_metabox' ) ) {
	/**
	 ...
	 * @return DIDO_Metabox
	 */
	function dido_metabox( $id ) {
		return DIDO_Metabox::instance( $id );
	}
}
