<?php
/**
 * AJ Plugin Gradients Class.
 *
 * @class   DIDO_Gradients
 * @package AJ\PluginFramework\Classes
 */

defined( 'ABSPATH' ) || exit; // Exit if accessed directly.

if ( ! class_exists( 'DIDO_Gradients' ) ) {

	/**
	 * DIDO_Gradients class.
	 *
	 * @deprecated 3.5
	 */
	class DIDO_Gradients {
		 ...
		 *
		 * @return mixed
		 * @deprecated 3.5 | use DIDO_Gradients::get_model instead
		 */
		...
		public function get_model( $class ) {
			return class_exists( 'DIDO_Registry' ) ? DIDO_Registry::get_instance()->$class : false;
		}
		...
		public function __get( $prop ) {
			$value = null;
			if ( 'request' === $prop ) {
				if ( class_exists( 'DIDO_Request' ) && ( ! isset( $this->request ) || ! $this->request instanceof DIDO_Request ) ) {
					$value         = DIDO_Registry::get_instance()->request;
					$this->request = $value;
				}
			}
