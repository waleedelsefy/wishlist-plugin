<?php
/**
 * AJ System Status Class
 * handle System Status panel
 *
 * @class   AJ_System_Status
 * @package AJ\PluginFramework\Classes
 */

defined( 'ABSPATH' ) || exit; // Exit if accessed directly.

if ( ! class_exists( 'AJ_System_Status' ) ) {
	/**
	 * AJ_System_Status class.
	 *
	...
	class AJ_System_Status {
		...
		public function __construct() {
			 ...
			 * wp-admin/admin-post.php?page=aj_system_info
			 */
			...
			add_action( 'wp_ajax_aj_create_log_file', array( $this, 'create_log_file' ) );
		...
		public function set_requirements_labels() {
			...
			$this->requirement_labels = array(
				'min_wp_version'    => esc_html__( 'WordPress Version', 'aj-plugin-fw' ),
				'min_wc_version'    => esc_html__( 'WooCommerce Version', 'aj-plugin-fw' ),
				'wp_memory_limit'   => esc_html__( 'Available Memory', 'aj-plugin-fw' ),
				'min_php_version'   => esc_html__( 'PHP Version', 'aj-plugin-fw' ),
				'min_tls_version'   => esc_html__( 'TLS Version', 'aj-plugin-fw' ),
				'wp_cron_enabled'   => esc_html__( 'WordPress Cron', 'aj-plugin-fw' ),
				'simplexml_enabled' => esc_html__( 'SimpleXML', 'aj-plugin-fw' ),
				'mbstring_enabled'  => esc_html__( 'MultiByte String', 'aj-plugin-fw' ),
				'imagick_version'   => esc_html__( 'ImageMagick Version', 'aj-plugin-fw' ),
				'gd_enabled'        => esc_html__( 'GD Library', 'aj-plugin-fw' ),
				'iconv_enabled'     => esc_html__( 'Iconv Module', 'aj-plugin-fw' ),
				'opcache_enabled'   => esc_html__( 'OPCache Save Comments', 'aj-plugin-fw' ),
				'url_fopen_enabled' => esc_html__( 'URL FOpen', 'aj-plugin-fw' ),
			);

		}

		/**
		 * Add "System Information" submenu page under AJ Plugins
		 *
		 * @return void
		 * @since  1.0.0
		 * @author Alberto Ruggiero
		...
		public function add_submenu_page() {
			...
			$system_info  = get_option( 'aj_system_info', array() );
			$error_notice = ( isset( $system_info['errors'] ) && true === $system_info['errors'] ? ' <span class="aj-system-info-menu update-plugins">!</span>' : '' );
			$settings     = array(
				'parent_page' => 'aj_plugin_panel',
				'page_title'  => esc_html__( 'System Status', 'aj-plugin-fw' ),
				'menu_title'  => esc_html__( 'System Status', 'aj-plugin-fw' ) . $error_notice,
		...
		/**
		 * Add "System Information" page template under AJ Plugins
		 *
		 * @return void
		 * @since  1.0.0
		 * @author Alberto Ruggiero
		...
		public function show_information_panel() {
			...
			$path = defined( 'DIDO_CORE_PLUGIN_PATH' ) ? DIDO_CORE_PLUGIN_PATH : get_template_directory() . '/core/plugin-fw/';
		...
		public function check_system_status() {
			...
			if ( '' === get_option( 'aj_system_info' ) || ( isset( $_GET['page'] ) && $_GET['page'] === $this->page ) ) { //phpcs:ignore

				$this->add_requirements(
					esc_html__( 'AJ Plugins', 'aj-plugin-fw' ),
					array(
						'min_wp_version'  => '5.3',
						'min_wc_version'  => '4.2',
				...
				$this->add_requirements(
					esc_html__( 'WooCommerce', 'aj-plugin-fw' ),
					array(
						'wp_memory_limit' => '64M',
					)
				);
		...
		public function enqueue_scripts() {
			...
			wp_register_script( 'aj-system-info', dido_load_js_file( $script_path . '/assets/js/aj-system-info.js' ), array( 'jquery' ), '1.0.0', true );

			if ( isset( $_GET['page'] ) && 'aj_system_info' === $_GET['page'] ) { //phpcs:ignore
				wp_enqueue_style( 'dido-plugin-style' );
				wp_enqueue_style( 'aj-plugin-fw-fields' );
				wp_enqueue_script( 'aj-system-info' );

				$params = array(
					'ajax_url' => admin_url( 'admin-ajax.php' ),
				);

				wp_localize_script( 'aj-system-info', 'aj_sysinfo', $params );
		...
		public function activate_system_notice() {
			...
			if ( ( isset( $_GET['page'] ) && $_GET['page'] === $this->page ) || ( ! empty( $_COOKIE['hide_aj_system_alert'] ) && 'yes' === $_COOKIE['hide_aj_system_alert'] ) || ( '' === $system_info ) || ( '' !== $system_info && false === $system_info['errors'] ) ) { //phpcs:ignore
				return;
			}
			...
			if ( true === $show_notice ) {
				wp_enqueue_script( 'aj-system-info' );
				?>
				<div id="aj-system-alert" class="notice notice-error is-dismissible" style="position: relative;">
					<p>
						<span class="aj-logo"><img src="<?php echo esc_attr( aj_plugin_fw_get_default_logo() ); ?>"/></span>
						<b>
							<?php esc_html_e( 'Warning!', 'aj-plugin-fw' ); ?>
						</b><br/>
						<?php
						/* translators: %1$s open link tag, %2$s open link tag*/
						echo sprintf( esc_html__( 'The system check has detected some compatibility issues on your installation.%1$sClick here%2$s to know more', 'aj-plugin-fw' ), '<a href="' . esc_url( add_query_arg( array( 'page' => $this->page ), admin_url( 'admin.php' ) ) ) . '">', '</a>' );
						?>
					</p>
		...
		public function get_system_info() {
			...
			return apply_filters(
				...
				array(
					...
					'wp_cron_enabled'   => ( ! ( defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON ) || apply_filters( 'aj_system_status_server_cron', false ) ),
		...
		public function format_requirement_value( $key, $value ) {
			...
			if ( strpos( $key, '_enabled' ) !== false ) {
				echo esc_attr( $value ) ? esc_html__( 'Enabled', 'aj-plugin-fw' ) : esc_html__( 'Disabled', 'aj-plugin-fw' );
			} elseif ( 'wp_memory_limit' === $key ) {
				echo esc_html( size_format( $value ) );
			} else {
				if ( 'n/a' === $value ) {
					echo esc_html__( 'N/A', 'aj-plugin-fw' );
				} else {
					echo esc_attr( $value );
				}
			}
				...
				<?php foreach ( $item['errors'] as $plugin => $requirement ) : ?>
					<li>
						...
						if ( strpos( $key, '_enabled' ) !== false ) {
							...
							echo sprintf( esc_html__( '%1$s needs %2$s enabled', 'aj-plugin-fw' ), '<b>' . esc_attr( $plugin ) . '</b>', '<b>' . esc_attr( $label ) . '</b>' );
						} elseif ( 'wp_memory_limit' === $key ) {
							/* translators: %1$s plugin name, %2$s required memory amount */
							echo sprintf( esc_html__( '%1$s needs at least %2$s of available memory', 'aj-plugin-fw' ), '<b>' . esc_attr( $plugin ) . '</b>', '<span class="error">' . esc_html( size_format( $this->memory_size_to_num( $requirement ) ) ) . '</span>' );
						} else {
							/* translators: %1$s plugin name, %2$s version number */
							echo sprintf( esc_html__( '%1$s needs at least %2$s version', 'aj-plugin-fw' ), '<b>' . esc_attr( $plugin ) . '</b>', '<span class="error">' . esc_attr( $requirement ) . '</span>' );
						}
						?>
					</li>
				<?php endforeach; ?>
		...
		public function print_solution_suggestion( $key, $item, $label ) {
			switch ( $key ) {
				case 'min_wp_version':
				case 'min_wc_version':
					esc_html_e( 'Update it to the latest version in order to benefit of all new features and security updates.', 'aj-plugin-fw' );
					break;
				case 'min_php_version':
				case 'min_tls_version':
					esc_html_e( 'Contact your hosting company in order to update it.', 'aj-plugin-fw' );
					break;
				case 'imagick_version':
					if ( 'n/a' === $item['value'] ) {
						esc_html_e( 'Contact your hosting company in order to install it.', 'aj-plugin-fw' );
					} else {
						esc_html_e( 'Contact your hosting company in order to update it.', 'aj-plugin-fw' );
					}
					break;
				case 'wp_cron_enabled':
					...
					echo sprintf( esc_html__( 'Remove %1$s from %2$s file', 'aj-plugin-fw' ), '<code>define( \'DISABLE_WP_CRON\', true );</code>', '<b>wp-config.php</b>' );
					break;
				case 'mbstring_enabled':
				case 'simplexml_enabled':
				case 'gd_enabled':
				...
				case 'opcache_enabled':
				case 'url_fopen_enabled':
					esc_html_e( 'Contact your hosting company in order to enable it.', 'aj-plugin-fw' );
					break;
				case 'wp_memory_limit':
					/* translators: %1$s opening link tag, %2$s closing link tag */
					echo sprintf( esc_html__( 'Read more %1$shere%2$s or contact your hosting company in order to increase it.', 'aj-plugin-fw' ), '<a href="https://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP" target="_blank">', '</a>' );
					break;
				default:
					echo esc_attr( apply_filters( 'aj_system_generic_message', '', $key, $item, $label ) );
			}
		}
		...
		public function print_warning_messages( $key ) {
			switch ( $key ) {
				case 'wp_memory_limit':
					...
					echo sprintf( esc_html__( 'For optimal functioning of our plugins, we suggest setting at least %s of available memory', 'aj-plugin-fw' ), '<span class="warning">' . esc_html( size_format( $this->recommended_memory ) ) . '</span>' );
					echo '<br/>';
					/* translators: %1$s opening link tag, %2$s closing link tag */
					echo sprintf( esc_html__( 'Read more %1$shere%2$s or contact your hosting company in order to increase it.', 'aj-plugin-fw' ), '<a href="https://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP" target="_blank">', '</a>' );
					break;
				case 'min_tls_version':
					if ( ! function_exists( 'curl_init' ) ) {
						...
						echo sprintf( esc_html__( 'The system check cannot determine which %1$s version is installed because %2$s module is disabled. Ask your hosting company to enable it.', 'aj-plugin-fw' ), '<b>TLS</b>', '<b>cURL</b>' );
					} else {
						/* translators: %1$s TLS label */
						echo sprintf( esc_html__( 'The system check cannot determine which %1$s version is installed due to a connection issue between your site and our server.', 'aj-plugin-fw' ), '<b>TLS</b>' );
					}
					break;
			}
		}
		...
		public function get_tls_version() {
			$tls = get_transient( 'aj-plugin-fw-system-status-tls-version' );

			if ( ! $tls && apply_filters( 'aj_system_status_check_ssl', true ) ) {
				$services = array(
					array(
					...
					array(
						'url'              => 'https://ttl-version.ajemes.workers.dev/',
				...
				set_transient( 'aj-plugin-fw-system-status-tls-version', $tls, 300 );
			}
		...
		public function get_output_ip() {
			$ip = get_transient( 'aj-plugin-fw-system-status-output-ip' );

			if ( ! $ip && apply_filters( 'aj_system_status_check_ip', true ) ) {
				...
				set_transient( 'aj-plugin-fw-system-status-output-ip', $ip, 300 );
			}
...
if ( ! function_exists( 'AJ_System_Status' ) ) {
	/**
	 * Single instance of AJ_System_Status
	 *
	 * @return AJ_System_Status
	 * @since  1.0
	 * @author Alberto Ruggiero
	 */
	function AJ_System_Status() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
		return AJ_System_Status::instance();
	}
}
