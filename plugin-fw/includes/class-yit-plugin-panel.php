<?php
/**
 * AJ Plugin Panel Class.
 *
 * @class   DIDO_Plugin_Panel
 * @package AJ\PluginFramework\Classes
 */

defined( 'ABSPATH' ) || exit; // Exit if accessed directly.

if ( ! class_exists( 'DIDO_Plugin_Panel' ) ) {
	/**
	 * Class DIDO_Plugin_Panel
	 */
	class DIDO_Plugin_Panel {
		...
		public function __construct( $args = array() ) {
			if ( ! empty( $args ) ) {
				$default_args = array(
					...
					'page_title'  => __( 'Plugin Settings', 'aj-plugin-fw' ),
					'menu_title'  => __( 'Settings', 'aj-plugin-fw' ),
					'capability'  => 'manage_options',
					'icon_url'    => '',
					'position'    => null,
				);

				$args = apply_filters( 'dido_plugin_fw_panel_option_args', wp_parse_args( $args, $default_args ) );
			...
			add_action( 'aj_plugin_fw_before_aj_panel', array( $this, 'add_plugin_banner' ), 10, 1 );
			add_action( 'wp_ajax_aj_plugin_fw_save_toggle_element', array( $this, 'save_toggle_element_options' ) );
		}
		 ...
		 *
		 * @return bool
		 * @see      DIDO_Plugin_Panel::init_wp_with_tabs
		 * @since    3.4.17
		 */
		...
		/**
		 * Init actions to show AJ Panel tabs in WP Pages
		 *
		...
		public function init_wp_with_tabs() {
			...
			if ( $tabs ) {
				...
				if ( ! $is_block_editor ) {
					...
					wp_enqueue_script( 'aj-plugin-fw-wp-pages' );
				}
		...
		protected function maybe_init_vars() {
			if ( ! isset( $this->main_array_options ) && ! isset( $this->tabs_hierarchy ) ) {
				...
				foreach ( $this->settings['admin-tabs'] as $item => $v ) {
					...
					$path = apply_filters( 'aj_plugin_panel_item_options_path', $path, $options_path, $item, $this );
					if ( file_exists( $path ) ) {
						$_tab                     = include $path;
						...
						foreach ( $sub_tabs as $sub_item => $sub_options ) {
							...
							$sub_tab_path = apply_filters( 'aj_plugin_panel_sub_tab_item_options_path', $sub_tab_path, $sub_tabs, $sub_item, $this );
		...
		/**
		 * Add aj-plugin-fw-panel in body classes in Panel pages
		 *
		...
		public static function add_body_class( $admin_body_classes ) {
			global $pagenow;
			if ( ( 'admin.php' === $pagenow && strpos( get_current_screen()->id, 'aj-plugins_page' ) !== false ) ) {
				$admin_body_classes = ! substr_count( $admin_body_classes, ' aj-plugin-fw-panel ' ) ? $admin_body_classes . ' aj-plugin-fw-panel ' : $admin_body_classes;
			}
		...
		public function add_menu_page() {
			...
			if ( ! isset( $admin_page_hooks['aj_plugin_panel'] ) ) {
				...
				$capability = apply_filters( 'dido_plugin_panel_menu_page_capability', 'manage_options' );
				$show       = apply_filters( 'dido_plugin_panel_menu_page_show', true );

				// AJ text must NOT be translated.
				if ( ! ! $show ) {
					add_menu_page( 'aj_plugin_panel', 'AJ', $capability, 'aj_plugin_panel', null, aj_plugin_fw_get_default_logo(), $position );
		 ...
		 * Submenu page hack: Remove the duplicate DIDO Plugin link on subpages
		 *
		 * @author   Andrea Grillo <andrea.grillo@ajemes.com>
		 */
		...
		public function admin_enqueue_scripts() {
			...
			if ( 'admin.php' === $pagenow && strpos( get_current_screen()->id, $this->settings['page'] ) !== false || apply_filters( 'dido_plugin_panel_asset_loading', false ) ) {
				wp_enqueue_media();
				...
				wp_enqueue_script( 'aj_how_to' );
				wp_enqueue_script( 'aj-plugin-fw-fields' );
			}

			if ( ( 'admin.php' === $pagenow && aj_plugin_fw_is_panel() ) || apply_filters( 'dido_plugin_panel_asset_loading', false ) ) {
				wp_enqueue_media();
				wp_enqueue_style( 'dido-plugin-style' );
				wp_enqueue_script( 'dido-plugin-panel' );
			}

			if ( 'admin.php' === $pagenow && strpos( get_current_screen()->id, 'aj_upgrade_premium_version' ) !== false ) {
		...
		public function register_settings() {
			register_setting( 'dido_' . $this->settings['parent'] . '_options', 'dido_' . $this->settings['parent'] . '_options', array( $this, 'options_validate' ) );
		}
		...
		public function add_setting_page() {
			...
			if ( ! empty( $parent ) ) {
				add_submenu_page( $parent, $this->settings['page_title'], $this->settings['menu_title'], $this->settings['capability'], $this->settings['page'], array( $this, 'dido_panel' ) );
			} else {
				add_menu_page( $this->settings['page_title'], $this->settings['menu_title'], $this->settings['capability'], $this->settings['page'], array( $this, 'dido_panel' ), $this->settings['icon_url'], $this->settings['position'] );
			}
			// Duplicate Items Hack.
			$this->remove_duplicate_submenu_page();
			do_action( 'dido_after_add_settings_page' );
		...
		public function options_validate( $field ) {
			...
			foreach ( $dido_options[ $option_key ] as $section => $data ) {
		...
		public function add_premium_version_upgrade_to_menu() {
			...
			if ( function_exists( 'DIDO_Plugin_Licence' ) && ! ! DIDO_Plugin_Licence()->get_products() ) {
				return;
			}

			global $submenu;
			if ( apply_filters( 'dido_show_upgrade_to_premium_version', isset( $submenu['aj_plugin_panel'] ) ) ) {
				$how_to_menu                            = array(
					sprintf( '%s%s%s', '<span id="aj-how-to-premium">', __( 'How to install premium version', 'aj-plugin-fw' ), '</span>' ),
					'install_plugins',
					'//support.ajemes.com/hc/en-us/articles/217840988',
					__( 'How to install premium version', 'aj-plugin-fw' ),
				);
				$submenu['aj_plugin_panel']['how_to'] = $how_to_menu; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
			}
		}
		...
		public function print_tabs_nav( $args = array() ) {
			$defaults = array(
				...
				'premium_class' => isset( $this->settings['class'] ) ? 'aj-premium' : 'premium',
				'page'          => $this->settings['page'],
			...
			list ( $current_tab, $premium_class, $page, $parent_page, $wrapper_class ) = aj_plugin_fw_extract( $args, 'current_tab', 'premium_class', 'page', 'parent_page', 'wrapper_class' );

			$tabs = '<ul class="aj-plugin-fw-tabs">';
			...
			foreach ( $this->settings['admin-tabs'] as $tab => $tab_value ) {
				...
				$active_class = apply_filters( 'aj_plugin_fw_panel_active_tab_class', $active_class, $current_tab, $tab );
				...
				$icon          = ( $current_tab !== $tab && $sub_tabs ) ? '<i class="aj-icon aj-icon-arrow_down"></i>' : '';

				$tabs .= '<li class="aj-plugin-fw-tab-element">';
		...
		public function get_nav_url( $page, $tab, $sub_tab = '', $parent_page = '' ) {
			...
			return apply_filters( 'aj_plugin_fw_panel_url', $url, $page, $tab, $sub_tab, $parent_page );
		}
		...
		public function print_sub_tabs_nav( $args = array() ) {
			...
			if ( $sub_tabs && $current_sub_tab ) {
				include DIDO_CORE_PLUGIN_TEMPLATE_PATH . '/panel/sub-tabs-nav.php';
			}
		}
		...
		public function dido_panel() {
			...
			<div class="wrap <?php echo esc_attr( $wrap_class ); ?>">
				...
				<?php
				do_action( 'aj_plugin_fw_before_aj_panel', $this->settings['page'] );
						...
						<?php if ( $this->is_show_form() ) : ?>
							<form id="aj-plugin-fw-panel" method="post" action="options.php">
								...
								<input type="submit" class="button-primary"
										value="<?php esc_attr_e( 'Save Changes', 'aj-plugin-fw' ); ?>"
							...
							<form method="post">
								...
								<input type="submit" name="dido-reset" class="button-secondary"
										value="<?php esc_attr_e( 'Reset to default', 'aj-plugin-fw' ); ?>"
		...
		public function print_custom_tab( $options ) {
			...
			include DIDO_CORE_PLUGIN_TEMPLATE_PATH . '/panel/custom-tab.php';
		}
		...
		public function add_fields() {
			...
			foreach ( $dido_options[ $option_key ] as $section => $data ) {
				add_settings_section( "dido_settings_{$option_key}_{$section}", $this->get_section_title( $section ), $this->get_section_description( $section ), 'dido' );
				foreach ( $data as $option ) {
					if ( isset( $option['id'] ) && isset( $option['type'] ) && isset( $option['name'] ) ) {
						add_settings_field(
							'dido_setting_' . $option['id'],
							$option['name'],
							array( $this, 'render_field' ),
							'dido',
							"dido_settings_{$option_key}_{$section}",
							array(
								'option'    => $option,
		...
		public function get_current_tab() {
			...
			if ( isset( $_REQUEST['dido_tab_options'] ) ) {
				return sanitize_key( wp_unslash( $_REQUEST['dido_tab_options'] ) );
		...
		public function message() {
			// phpcs:disable WordPress.Security.NonceVerification.Recommended
			$message = array(
				'element_exists'   => $this->get_message( '<strong>' . __( 'The element you have entered already exists. Please, enter another name.', 'aj-plugin-fw' ) . '</strong>', 'error', false ),
				'saved'            => $this->get_message( '<strong>' . __( 'Settings saved', 'aj-plugin-fw' ) . '.</strong>', 'updated', false ),
				'reset'            => $this->get_message( '<strong>' . __( 'Settings reset', 'aj-plugin-fw' ) . '.</strong>', 'updated', false ),
				'delete'           => $this->get_message( '<strong>' . __( 'Element deleted correctly.', 'aj-plugin-fw' ) . '</strong>', 'updated', false ),
				'updated'          => $this->get_message( '<strong>' . __( 'Element updated correctly.', 'aj-plugin-fw' ) . '</strong>', 'updated', false ),
				'settings-updated' => $this->get_message( '<strong>' . __( 'Element updated correctly.', 'aj-plugin-fw' ) . '</strong>', 'updated', false ),
				'imported'         => $this->get_message( '<strong>' . __( 'Database imported correctly.', 'aj-plugin-fw' ) . '</strong>', 'updated', false ),
				'no-imported'      => $this->get_message( '<strong>' . __( 'An error has occurred during import. Please try again.', 'aj-plugin-fw' ) . '</strong>', 'error', false ),
				'file-not-valid'   => $this->get_message( '<strong>' . __( 'The added file is not valid.', 'aj-plugin-fw' ) . '</strong>', 'error', false ),
				'cant-import'      => $this->get_message( '<strong>' . __( 'Sorry, import is disabled.', 'aj-plugin-fw' ) . '</strong>', 'error', false ),
				'ord'              => $this->get_message( '<strong>' . __( 'Sorting successful.', 'aj-plugin-fw' ) . '</strong>', 'updated', false ),
			);
		...
		public function get_default_options() {
			...
			foreach ( $dido_options as $tab => $sections ) {
		...
		public function get_tab_title() {
			...
			foreach ( $dido_options[ $option_key ] as $sections => $data ) {
				foreach ( $data as $option ) {
		...
		public function get_section_title( $section ) {
			...
			foreach ( $dido_options[ $option_key ][ $section ] as $option ) {
		...
		public function get_name_field( $name = '' ) {
			return 'dido_' . $this->settings['parent'] . '_options[' . $name . ']';
		}
		...
		public function get_id_field( $id ) {
			return 'dido_' . $this->settings['parent'] . '_options_' . $id;
		}
		...
		public function render_field( $param ) {
			if ( ! empty( $param ) && isset( $param ['option'] ) ) {
				...
				$field_template_path = aj_plugin_fw_get_field_template_path( $option );
				if ( $field_template_path ) {
					$field_container_path = apply_filters( 'aj_plugin_fw_panel_field_container_template_path', DIDO_CORE_PLUGIN_TEMPLATE_PATH . '/panel/panel-field-container.php', $option );
					file_exists( $field_container_path ) && include $field_container_path;
				} else {
					do_action( "dido_panel_{$option['type']}", $option, $db_value, $custom_attributes );
				}
			}
		}
		...
		public function get_options() {
			$options = get_option( 'dido_' . $this->settings['parent'] . '_options' );
			if ( false === $options || ( isset( $_REQUEST['dido-action'] ) && 'reset' === sanitize_key( wp_unslash( $_REQUEST['dido-action'] ) ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		public static function add_infobox( $args = array() ) {
			if ( ! empty( $args ) ) {
				extract( $args ); // phpcs:ignore WordPress.PHP.DontExtract.extract_extract
				require DIDO_CORE_PLUGIN_TEMPLATE_PATH . '/panel/boxinfo.php';
			}
		}
		...
		public static function sort_plugins() {
			global $submenu;
			if ( ! empty( $submenu['aj_plugin_panel'] ) ) {
				$sorted_plugins = $submenu['aj_plugin_panel'];
				...
				$submenu['aj_plugin_panel'] = $sorted_plugins; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
			}
		}

		/**
		 * Add menu class in AJ Plugins menu.
		 *
		 ...
		 * @author   Leanza Francesco <leanzafrancesco@gmail.com>
		 */
		public static function add_menu_class_in_aj_plugin( $menu ) {
			global $submenu;

			if ( ! empty( $submenu['aj_plugin_panel'] ) ) {
				$item_count = count( $submenu['aj_plugin_panel'] );
				$columns    = absint( $item_count / 20 ) + 1;
				$columns    = max( 1, min( $columns, 3 ) );
				$columns    = apply_filters( 'aj_plugin_fw_aj_plugins_menu_columns', $columns, $item_count );

				if ( $columns > 1 ) {
					$class = "aj-plugin-fw-menu-$columns-columns";
					foreach ( $menu as $order => $top ) {
						if ( 'aj_plugin_panel' === $top[2] ) {
							$c                 = $menu[ $order ][4];
		...
		public function add_plugin_banner( $page ) {
			...
			<?php if ( $this->is_free() && isset( $this->settings['plugin_slug'] ) ) : ?>
				<?php
				$rate_link = apply_filters( 'aj_plugin_fw_rate_url', 'https://wordpress.org/support/plugin/' . $this->settings['plugin_slug'] . '/reviews/#new-post' );
				?>
				<h1 class="notice-container"></h1>
				<div class="aj-plugin-fw-banner">
					<h1><?php echo esc_html( $this->settings['page_title'] ); ?></h1>
				</div>
				<div class="aj-plugin-fw-rate">
					<?php
					printf(
						...
						esc_html__( 'We need your support', 'aj-plugin-fw' ),
						esc_html__( 'to keep updating and improving the plugin. Please,', 'aj-plugin-fw' ),
						esc_url( $rate_link ),
						esc_html__( 'help us by leaving a five-star rating', 'aj-plugin-fw' ),
						esc_html__( ':) Thanks!', 'aj-plugin-fw' )
					);
					?>
				</div>
			<?php else : ?>
		 ...
		 * @since  3.2
		 */
		public function add_aj_ui( $field ) {
			global $pagenow;
			...
			if ( 'admin.php' === $pagenow && $screen && strpos( $screen->id, $this->settings['page'] ) !== false ) {
				switch ( $field['type'] ) {
					case 'datepicker':
						echo '<span class="aj-icon aj-icon-calendar"></span>';
						break;
					default:
						break;
				}
		...
		public function get_post_type_tabs( $post_type ) {
			...
			return apply_filters( "aj_plugin_fw_panel_{$panel_page}_get_post_type_tabs", $tabs, $post_type );
		}
		...
		public function get_taxonomy_tabs( $taxonomy ) {
			...
			return apply_filters( "aj_plugin_fw_panel_{$panel_page}_get_taxonomy_tabs", $tabs, $taxonomy );
		}
		...
		public function maybe_redirect_to_proper_wp_page() {
			...
			if (
				...
				&&
				! isset( $_REQUEST['aj-plugin-fw-panel-skip-redirect'] )
			) {
				...
				if ( strpos( $url, 'edit.php' ) !== false || strpos( $url, 'edit-tags.php' ) !== false ) {
					wp_safe_redirect( add_query_arg( array( 'aj-plugin-fw-panel-skip-redirect' => 1 ), $url ) );
					exit;
				}
			}
			// phpcs:enable
		 ...
		 * Important: this opens a wrapper <div> that will be closed through DIDO_Plugin_Panel::print_panel_tabs_in_post_edit_page_end()
		 *
		 * @since    3.4.0
		 * @author   Leanza Francesco <leanzafrancesco@gmail.com>
		 */
		public function print_panel_tabs_in_wp_pages() {
			if ( self::$panel_tabs_in_wp_pages ) {
				...
				$wrap_class = isset( $this->settings['class'] ) ? $this->settings['class'] : '';
				?>
				<div class="aj-plugin-fw-wp-page-wrapper">
				<?php
		...
		/**
		 * Close the wrapper opened in DIDO_Plugin_Panel::print_panel_tabs_in_wp_pages()
		 *
		 * @since    3.4.0
		 * @author   Leanza Francesco <leanzafrancesco@gmail.com>
		 */
		public function print_panel_tabs_in_wp_pages_end() {
			if ( self::$panel_tabs_in_wp_pages ) {
				echo '</div><!-- /aj-plugin-fw-wp-page-wrapper -->';
			}
		}
