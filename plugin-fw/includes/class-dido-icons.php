...
if ( ! class_exists( 'DIDO_Icons' ) ) {
	...
	class DIDO_Icons {
		 ...
		 *
		 * @return DIDO_Icons
		 * @deprecated 3.5 | use DIDO_Icons::get_instance() instead.
		 */
		...
		public function get_icons( $filter_suffix = '' ) {
			...
			if ( ! empty( $filter_suffix ) ) {
				$icons = apply_filters( 'aj_plugin_fw_icons_field_icons_' . sanitize_key( $filter_suffix ), $icons );
			}
		...
		private function get_font_awesome_array() {
			$font_awesome_list    = get_option( 'dido_font_awesome_list', array() );
			...
			if ( ! empty( $font_awesome_list[ $font_awesome_version ] ) ) {
				...
				if ( ! is_wp_error( $response ) && 200 === absint( $response['response']['code'] ) && 'OK' === $response['response']['message'] ) {
					...
					update_option( 'dido_font_awesome_list', $font_awesome_list );
				}
			}
		...
		private function get_dashicons_array() {
			$dashicons_list = get_option( 'dido_dashicons_list', array() );
		 ...
		 *
		 * @return string
		 * @deprecated 3.5 | use DIDO_Icons::get_inline_style()
		 */
...
if ( ! function_exists( 'dido_icons' ) ) {
	/**
	 * Return instance of DIDO_Icons class.
	 *
	 * @return DIDO_Icons
	 */
	function dido_icons() {
		return DIDO_Icons::instance();
	}
}
