...
if ( ! class_exists( 'DIDO_Pointers' ) ) {
	...
	class DIDO_Pointers {
		...
		private function __construct() {
			...
			$title            = __( 'Plugins Activated', 'aj-plugin-fw' );
			$message          = __( 'From now on, you can find all plugin options in AJ menu. Plugin customization settings will be available as a new entry in AJ menu.', 'aj-plugin-fw' );
			$discover_message = sprintf(
			// translators: 1. AJ site link; 2. WordPress site link.
				__( 'Discover all our plugins available on: %1$s and %2$s', 'aj-plugin-fw' ),
				'<a href="https://ajemes.com/product-category/plugins/" target="_blank">ajemes.com</a>',
				'<a href="https://profiles.wordpress.org/ajemes/" target="_blank">Wordpress.org</a>'
			);
			...
			$title            = __( 'Plugins Upgraded', 'aj-plugin-fw' );
			$message          = __( 'From now on, you can find the option panel of AJ plugins in AJ menu. Every time one of our plugins is added, a new entry will be added to this menu. For example, after the update, plugin options (such as for AJ WooCommerce Wishlist, AJ WooCommerce Ajax Search, etc.) will be moved from previous location to AJ menu.', 'aj-plugin-fw' );
			$discover_message = sprintf(
			// translators: 1. AJ site link; 2. WordPress site link.
				__( 'Discover all our plugins available on: %1$s and %2$s', 'aj-plugin-fw' ),
				'<a href="https://ajemes.com/product-category/plugins/" target="_blank">ajemes.com</a>',
			...
			// DEPRECATED 'dido-pointer-special-screen' filter since 3.5 | use aj_plugin_fw_pointers_special_screens instead.
			$this->special_screen = apply_filters( 'dido-pointer-special-screen', array( 'plugins', 'update' ) ); //phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
			$this->special_screen = apply_filters( 'aj_plugin_fw_pointers_special_screens', array( 'plugins', 'update' ) );
		...
		public function parse_args( $args ) {
			$default = array(
				'pointer_id' => 'aj_default_pointer',
				'target'     => '#toplevel_page_dido_plugin_panel',
				'init'       => null,
			);
		...
		public function add_pointers() {
			if ( ! empty( $this->screen_ids ) ) {
				foreach ( $this->screen_ids as $screen_id ) {
					add_filter( "dido_pointers-{$screen_id}", array( $this, 'pointers' ) );
				}
			}
		}
		...
		public function pointer_load( $deprecated = false ) {
			...
			$pointers = apply_filters( "dido_pointers-{$screen->id}", array() ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
			...
			if ( in_array( $screen->id, $this->special_screen, true ) ) {
				...
				$recently_activate = get_option( 'dido_recently_activated', array() );
				...
				if ( $is_single_activate || $is_single_upgrade ) {
					...
					if ( $is_multi_upgrade && isset( $_POST['checked'] ) && ( count( $_POST['checked'] ) > 0 ) ) {
						...
						$screen_id         = 'update';
						$pointers          = apply_filters( "dido_pointers-{$screen_id}", array() ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
					}
					 ...
					 * count( $point_id ) is the number of AJ plugins that have registered specific pointers
				...
				update_option( 'dido_recently_activated', array() );
			...
			$script_file = function_exists( 'dido_load_js_file' ) ? dido_load_js_file( 'dido-wp-pointer.js' ) : 'dido-wp-pointer.min.js';

			// Enqueue pointer scripts and styles.
			wp_enqueue_style( 'wp-pointer' );
			wp_enqueue_script( 'wp-pointer' );

			wp_enqueue_script( 'dido-wp-pointer', DIDO_CORE_PLUGIN_URL . '/assets/js/' . $script_file, array( 'wp-pointer' ), aj_plugin_fw_get_version(), true );
			wp_localize_script( 'dido-wp-pointer', 'custom_pointer', $valid_pointers );
		...
		public function pointers( $pointers ) {
			$screen_id       = str_replace( 'dido_pointers-', '', current_filter() );
...
if ( ! function_exists( 'DIDO_Pointers' ) ) {

	/**
	 * Single instance of DIDO_Pointers
	 *
	 * @return DIDO_Pointers
	 */
	function DIDO_Pointers() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
		return DIDO_Pointers::instance();
	}
}
