...
if ( ! class_exists( 'AJ_Debug' ) ) {
	...
	class AJ_Debug {
		 ...
		 *
		 * @return AJ_Debug
		 * @deprecated 3.5 | use AJ_Debug::get_instance() instead.
		 */
		...
		public function init() {
			...
			$is_debug = apply_filters( 'aj_plugin_fw_is_debug', isset( $_GET['aj-debug'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		public function add_debug_in_admin_bar( $wp_admin_bar ) {
			$args = array(
				'id'    => 'aj-debug-admin-bar',
				'title' => 'AJ Debug',
				'href'  => '',
				'meta'  => array(
					'class' => 'aj-debug-admin-bar',
				),
			);
			...
			foreach ( $this->get_debug_information() as $key => $information ) {
				...
				$subnodes[] = array(
					'id'     => 'aj-debug-admin-bar-' . $key,
					'parent' => 'aj-debug-admin-bar',
					'title'  => $title,
					'href'   => $url,
					'meta'   => array(
						'class' => 'aj-debug-admin-bar-' . $key,
					),
				);
				...
				if ( isset( $information['subsub'] ) ) {
					foreach ( $information['subsub'] as $sub_key => $sub_value ) {
						...
						$subnodes[] = array(
							'id'     => 'aj-debug-admin-bar-' . $key . '-' . $sub_key,
							'parent' => 'aj-debug-admin-bar-' . $key,
							'title'  => $title,
							'href'   => '',
							'meta'   => array(
								'class' => 'aj-debug-admin-bar-' . $key . '-' . $sub_key,
								'html'  => $html,
							),
						);
					}
		...
		public function get_debug_information() {
			...
			$debug = array(
				...
				'aj-premium-plugins' => array(
					'label'  => 'AJ Premium Plugins',
					'value'  => '',
				...
				'post-meta'            => array(
					'label' => 'Post Meta',
					'value' => '',
					'url'   => add_query_arg( array( 'aj-debug-post-meta' => 'all' ) ),
				),
				'option'               => array(
					'label' => 'Option',
					'value' => '',
					'url'   => add_query_arg( array( 'aj-debug-option' => '' ) ),
				),
			);

			// Post Meta debug.
			global $post;
			if ( ! empty( $_GET['aj-debug-post-meta'] ) && $post ) {
				$meta_key   = sanitize_key( wp_unslash( $_GET['aj-debug-post-meta'] ) );
			...
			// Option debug.
			if ( ! empty( $_GET['aj-debug-option'] ) ) {
				$option_key   = sanitize_key( wp_unslash( $_GET['aj-debug-option'] ) );
		...
		public function get_plugin_framework_info() {
			$plugin_fw_version   = aj_plugin_fw_get_version();
			$plugin_fw_loaded_by = basename( dirname( DIDO_CORE_PLUGIN_PATH ) );
		...
		public function get_premium_plugins_info() {
			$plugins      = DIDO_Plugin_Licence()->get_products();
			$plugins_info = array();
...
if ( ! function_exists( 'aj_debug' ) ) {
	/**
	 * Single instance of AJ_Debug
	 *
	 * @return AJ_Debug
	 */
	function aj_debug() {
		return AJ_Debug::instance();
	}
