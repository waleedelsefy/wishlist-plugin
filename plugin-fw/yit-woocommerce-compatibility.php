...
if ( ! function_exists( 'dido_get_prop' ) ) {
	/**
	 * Retrieve a property.
	 *
	 ...
	 */
	function dido_get_prop( $object, $key, $single = true, $context = 'view' ) {
		$prop_map   = dido_return_new_attribute_map();
		...
		if ( $is_wc_data ) {
			...
			if ( isset( $object->$key ) ) {
				return $object->$key;
			} elseif ( dido_wc_check_post_columns( $key ) ) {
...
if ( ! function_exists( 'dido_set_prop' ) ) {
	/**
	 * Set prop or props.
	 *
	 * @param object       $object The object.
	 ...
	 * @deprecated 3.5 | use the WooCommerce CRUD directly instead.
	 */
	function dido_set_prop( $object, $arg1, $arg2 = false ) {
		if ( ! is_array( $arg1 ) ) {
			$arg1 = array(
				$arg1 => $arg2,
			);
		...
		foreach ( $arg1 as $key => $value ) {
			if ( $is_wc_data ) {
				...
				if ( dido_wc_check_post_columns( $key ) ) {
...
if ( ! function_exists( 'dido_save_prop' ) ) {
	/**
	 * Save prop or props.
	 *
	 * @param object       $object       The object.
	 ...
	 * @deprecated 3.5 | use the WooCommerce CRUD directly instead.
	 */
	function dido_save_prop( $object, $arg1, $arg2 = false, $force_update = false ) {
		if ( ! is_array( $arg1 ) ) {
			$arg1 = array(
				$arg1 => $arg2,
			);
		...
		foreach ( $arg1 as $key => $value ) {
			dido_set_prop( $object, $key, $value );

			if ( ! $is_wc_data ) {

				if ( dido_wc_check_post_columns( $key ) ) {
					dido_store_changes( $object->post, $key, $value );
				} else {
					$object_id = is_callable( array( $object, 'get_id' ) ) ? $object->get_id() : $object->id;
...
if ( ! function_exists( 'dido_delete_prop' ) ) {
	/**
	 * Delete a prop.
	 *
	 * @param object $object The object.
	 ...
	 * @deprecated 3.5 | use the WooCommerce CRUD directly instead.
	 */
	function dido_delete_prop( $object, $key, $value = '' ) {
		$prop_map   = dido_return_new_attribute_map();
		$is_wc_data = $object instanceof WC_Data;
		...
		if ( $is_wc_data ) {
			...
			if ( dido_wc_check_post_columns( $key ) && ( ! $value || $object->post->$key === $value ) ) {
				dido_store_changes( $object->post, $key, '' );
			} else {
				$object_id = is_callable( array( $object, 'get_id' ) ) ? $object->get_id() : $object->id;
...
if ( ! function_exists( 'dido_return_new_attribute_map' ) ) {
	/**
	 ...
	 * @deprecated 3.5
	 */
	function dido_return_new_attribute_map() {
		return array(
...
if ( ! function_exists( 'dido_store_changes' ) ) {
	/**
	 * Store changes
	 *
	 * @param object      $object The object.
	 ...
	 * @deprecated 3.5
	 */
	function dido_store_changes( $object, $key, $value = false ) {
...
if ( ! function_exists( 'dido_send_changes_to_db' ) ) {
	/**
	 * Send changes to DB.
	 *
	 * @deprecated 3.5
	 */
	function dido_send_changes_to_db() {
		global $changed_objects;
		...
		if ( ! empty( $changed_objects ) ) {
			foreach ( $changed_objects as $id => $data ) {
				if ( version_compare( WC()->version, '2.7.0', '>=' ) ) {
					...
					dido_set_prop( $object, $data['changes'] );
					$object->save();
				} else {
					$data['ID'] = $id;
					wp_update_post( $data );
...
if ( ! function_exists( 'dido_get_orders' ) ) {
	/**
	 * Retrieve orders
	 *
...
if ( ! function_exists( 'dido_get_products' ) ) {
	/**
	 * Retrieve products.
	 *
	 * @param array $args Arguments.
	 ...
	 * @deprecated 3.5 | use wc_get_orders instead.
	 */
	function dido_get_products( $args ) {
		if ( version_compare( WC()->version, '2.7', '<' ) ) {
			$args['fields'] = 'objects';
			$posts          = get_posts( $args );
...
if ( ! function_exists( 'dido_update_product_stock' ) ) {
	/**
	 ...
	 */
	function dido_update_product_stock( $product, $stock_quantity = 1, $operation = 'set' ) {
...
if ( ! function_exists( 'dido_wc_deprecated_filters' ) ) {
	/**
	 * Deprecated filters.
	 *
	 * @return mixed|void
	 * @deprecated 3.5
	 */
	function dido_wc_deprecated_filters() {
		$filters = array(
		...
		return apply_filters( 'dido_wc_deprecated_filters', $filters );
	}
}

if ( ! function_exists( 'dido_fix_wc_deprecated_filters' ) ) {
	/**
	 * Fix WooCommerce deprecated filters.
	 *
	 * @deprecated 3.5
	 */
	function dido_fix_wc_deprecated_filters() {
		if ( ! version_compare( WC()->version, '2.7.0', '<' ) ) {
			return;
		}

		$deprecated_filters = dido_wc_deprecated_filters();
		foreach ( $deprecated_filters as $old => $new ) {
			add_filter( $old, 'dido_wc_deprecated_filter_mapping', 10, 100 );
		}
	}
}

if ( ! function_exists( 'dido_wc_deprecated_filter_mapping' ) ) {
	/**
	 ...
	 * @deprecated 3.5
	 */
	function dido_wc_deprecated_filter_mapping() {
		$deprecated_filters = dido_wc_deprecated_filters();
...
if ( ! function_exists( 'dido_wc_check_post_columns' ) ) {
	/**
	 ...
	 * @deprecated 3.5
	 */
	function dido_wc_check_post_columns( $key ) {
		$columns = array(
			'post_author',
			'post_date',
			'post_date_gmt',
...
if ( ! function_exists( 'dido_get_order_id' ) ) {
	/**
	 ...
	 */
	function dido_get_order_id( $order ) {
		return dido_get_prop( $order, 'id' );
	}
}

if ( ! function_exists( 'dido_get_product_id' ) ) {
	/**
	 ...
	 */
	function dido_get_product_id( $product ) {
		return dido_get_prop( $product, 'id' );
	}
}

if ( ! function_exists( 'dido_get_base_product_id' ) ) {
	/**
	 ...
	 * @return int
	 */
	function dido_get_base_product_id( $product ) {

		return $product instanceof WC_Data && $product->is_type( 'variation' ) ?
			dido_get_prop( $product, 'parent_id' ) :
			dido_get_prop( $product, 'id' );
	}
}

if ( ! function_exists( 'dido_get_display_price' ) ) {
	/**
	 ...
	 */
	function dido_get_display_price( $product, $price = '', $qty = 1 ) {
		if ( version_compare( WC()->version, '2.7.0', '>=' ) ) {
...
if ( ! function_exists( 'dido_get_price_excluding_tax' ) ) {
	/**
	 ...
	 * @return float|string
	 */
	function dido_get_price_excluding_tax( $product, $qty = 1, $price = '' ) {
		if ( version_compare( WC()->version, '2.7.0', '>=' ) ) {
...
if ( ! function_exists( 'dido_get_price_including_tax' ) ) {
	/**
	 ...
	 * @return float|string
	 */
	function dido_get_price_including_tax( $product, $qty = 1, $price = '' ) {
		if ( version_compare( WC()->version, '2.7.0', '>=' ) ) {
...
if ( ! function_exists( 'dido_get_product_image_id' ) ) {
	/**
	 ...
	 */
	function dido_get_product_image_id( $product, $context = 'view' ) {
		if ( version_compare( WC()->version, '2.7.0', '>=' ) ) {
...
if ( ! function_exists( 'dido_get_refund_amount' ) ) {
	/**
	 ...
	 */
	function dido_get_refund_amount( $refunded_order, $context = 'view' ) {
...
if ( ! function_exists( 'dido_set_refund_amount' ) ) {
	/**
	 ...
	 */
	function dido_set_refund_amount( $refunded_order, $amount ) {
		$is_wc_data = $refunded_order instanceof WC_Data;
...
if ( ! function_exists( 'dido_get_refund_reason' ) ) {
	/**
	 ...
	 */
	function dido_get_refund_reason( $refunded_order ) {
		$is_wc_data = $refunded_order instanceof WC_Data;
...
if ( ! function_exists( 'dido_product_visibility_meta' ) ) {
	/**
	 ...
	 * @return array
	 */
	function dido_product_visibility_meta( $args ) {
		if ( version_compare( WC()->version, '2.7.0', '<' ) ) {
...
if ( ! function_exists( 'dido_datetime_to_timestamp' ) ) {
	/**
	 ...
	 * @return false|int
	 */
	function dido_datetime_to_timestamp( $date ) {
		if ( ! is_int( $date ) ) {
			$date = strtotime( $date );
		}
...
dido_fix_wc_deprecated_filters();
add_action( 'shutdown', 'dido_send_changes_to_db' );
